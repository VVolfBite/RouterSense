...
3. `O_joint` 相对 `O_local` 平均改善多少？
   在统一 canonical bucket-wave exact 模型下：mean 6.49%, median 0.00%, p90 20.00%, max 50.00%；9/32 个实例存在严格联合收益，机会子集平均改善 23.07%。
4. 正式 exact oracle 一共多少个 OPTIMAL 实例？
   32/32 Local 与 Joint 均 certified OPTIMAL；dominance violation 为 0。历史 CT/atomic CP-SAT 仅保留为敏感性结果。
...
10. 各启发式距离统一模型 `O_joint` 还有多远？
    - birkhoff_phase_local_optimality_gap_to_O_joint: mean gap 9.43%, median 0.00%.
    - joint_copy_current_optimality_gap_to_O_joint: mean gap 3.49%, median 0.00%.
    - joint_perfect_trace_hint_optimality_gap_to_O_joint: mean gap 0.52%, median 0.00%.
    - joint_zero_hint_optimality_gap_to_O_joint: mean gap 0.52%, median 0.00%.
    - phase_barrier_fifo_optimality_gap_to_O_joint: mean gap 9.43%, median 0.00%.
    - safe_copy_current_optimality_gap_to_O_joint: mean gap 2.34%, median 0.00%.
    - safe_perfect_trace_hint_optimality_gap_to_O_joint: mean gap 0.00%, median 0.00%.
    所有 gap 均为非负；不再出现 heuristic 优于 exact `O_joint` 的模型错配伪结果。
...
15. 哪些结论可以进入论文，哪些仍不能？
   can enter: unified same-model exact small-instance O_joint<=O_local evidence, joint heuristics beating FIFO/Birkhoff on the replay fixture, and copy-current beating zero-hint on average. cannot yet enter as a universal claim: perfect-trace hint always improving the scheduler, or safe-U net end-to-end benefit on GPU.
