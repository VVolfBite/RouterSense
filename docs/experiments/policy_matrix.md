...
- Formal `O_local` and `O_joint` use `routersense_exact_bucket_wave_release_v2` and differ only by scope.
- `birkhoff_von_neumann_fluid` is a separate fluid sensitivity reference.
- `B_birkhoff` / `birkhoff_phase_local` are strong engineering baselines, not formal exact oracles.
