> **Historical model note:** this study predates the unified canonical bucket-wave exact oracle. Its “current atomic oracle” discussion is retained for migration history and must not be used as the current formal `O_joint` definition.
