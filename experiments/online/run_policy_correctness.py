...
from rs.topology import resolve_runtime_link_cost_profile
...
def _build_online_runtime_config(config: RunConfig, *, planner_config: dict[str, Any] | None = None) -> OnlineRuntimeConfig:
    return OnlineRuntimeConfig(
        ...
        policy_parameters=OnlinePolicyParameters(
            planner_id=str(getattr(config.online_policy.parameters, "planner_id", "") or ""),
            ...
            p3_return_weight=config.online_policy.parameters.p3_return_weight,
            planning_horizon=config.online_policy.parameters.planning_horizon,
            planning_timing=config.online_policy.parameters.planning_timing,
...
def main(argv: list[str] | None = None) -> int:
    ...
    planner_config, link_cost_metadata = resolve_runtime_link_cost_profile(
        configured_path=(
            config.topology.cost_profile
            or os.environ.get("RS_LINK_COST_PROFILE", "")
        ),
        source_config_path=config.source_config_path,
        repository_root=ROOT,
        model_path=model_path,
        precision=str(config.runtime.precision),
        world_size=int(os.environ.get("WORLD_SIZE", str(config.topology.ep_size))),
        local_world_size=int(
            os.environ.get(
                "LOCAL_WORLD_SIZE",
                str(config.topology.launcher.nproc_per_node),
            )
        ),
        require_profile=bool(config.topology.require_cost_profile),
    )
    ...
    try:
        ...
        for iter_index in range(total_iters):
            ...
            try:
                ...
                repeat_record = {
                    ...
                    "joint_candidate_build_us": _delta_perf(perf_after, perf_before, "joint_candidate_build"),
                    "local_fallback_build_us": _delta_perf(perf_after, perf_before, "local_fallback_build"),
                    ...
                    "unattributed_us": max(
                        ...
                        - (
                            ...
                            + float(_delta_perf(perf_after, perf_before, "joint_candidate_build"))
                            + float(_delta_perf(perf_after, perf_before, "local_fallback_build"))
        ...
        audit_payload = _build_rank_audits(
            ...
            policy_enabled=bool(
                policy_name and online_runtime_config.execution_mode in {"phase_sync_wave", "joint_window_async_p2p"}
            ),
        )
