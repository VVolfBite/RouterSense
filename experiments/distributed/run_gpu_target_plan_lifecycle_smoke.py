...
def _build_target_plan(
    ...
    *,
    ...
    raw_policy = "future:p012:joint:global:rscf"
    paired_policy = "future:p012:local:global:rscf" if safe_projection_mode == "host_select" else ""
    bundle, plan = service._build_target_plan(  # noqa: SLF001
        request=TargetLayerPlanningRequest(
            ...
            joint_planner_id=raw_policy,
            local_planner_id=paired_policy,
            safe_projection_mode=safe_projection_mode,
...
def _run_cases(rank: int, local_rank: int, world_size: int) -> dict[str, Any]:
    ...
    return {
        ...
        "safe_ready": {
            ...
            "local_fallback_logical_plan_digest": str(getattr(safe_ready_plan, "local_fallback_logical_plan_digest", "")) if safe_ready_plan is not None else "",
        },
        "safe_late": {
            ...
            "local_fallback_logical_plan_digest": str(getattr(safe_late_plan, "local_fallback_logical_plan_digest", "")),
        },
    }
...
def _passed(gathered: list[dict[str, Any]]) -> bool:
    ...
    if not all(((item.get("terminal") or {}).get("final_status") == "CONSUMED") for item in safe_ready):
        return False
    if not all(bool(item.get("local_fallback_logical_plan_digest")) for item in safe_ready):
        return False
    ...
    if not all(int((item.get("hidden_async_summary") or {}).get("suffix_splice_count", 0) or 0) == 1 for item in safe_late):
        return False
    if not all(bool(item.get("local_fallback_logical_plan_digest")) for item in safe_late):
        return False
    return True
