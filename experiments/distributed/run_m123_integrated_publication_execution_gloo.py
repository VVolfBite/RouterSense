...
def _is_phase_independent_policy(policy_id: str) -> bool:
    name = str(policy_id)
    if is_axes_planner_id(name):
        return parse_planner_axes(name).scope == "local"
    return str(resolve_algorithm_id(name).spec.scheduling_scope).startswith("phase_local")


def _policy_identity_fields(window_plan) -> dict[str, object]:
    planner_id = str(window_plan.planner_id)
    resolved = PlannerRegistry.resolve(planner_id)
    algorithm_id = str(resolved.planner_id)
    policy_name = str(window_plan.metadata.get("source_policy_name", planner_id))
    common_core = dict(window_plan.metadata.get("common_core") or {})
    phase_independent = bool(
        common_core.get("phase_independent", False)
        or _is_phase_independent_policy(planner_id)
    )
    return {
...
def _publish_requested_window_plan_through_formal_store(
    *,
    ...
    if str(requested_policy_id) == "future:p012:joint:global:rscf":
        _wait_until(
            lambda: (
        ...
        if published_plan is None:
            ...
            if not prepared_payload:
                raise KeyError("missing prepared Joint publication payload")
            prepared_plan = TargetLayerPreparedJointPlan.from_dict(prepared_payload)
            if prepared_plan.window_plan is None:
                raise KeyError("missing prepared Joint window plan in publication payload")
    ...
    request = build_window_planning_request(
        identity=PlanningIdentity(
            request_id=f"{runtime.run_id}:{runtime._forward_epoch}:{runtime.microbatch_id}:{slot.target_layer_id}:formal_local",  # noqa: SLF001
            run_id=str(runtime.run_id),
            forward_id=str(runtime._forward_epoch),  # noqa: SLF001
            window_id=f"{runtime._forward_epoch}:{runtime.microbatch_id}:{slot.target_layer_id}:formal_local",  # noqa: SLF001
    ...
    prepared_plan = TargetLayerPreparedJointPlan(
        ...
        selected_variant="local_fallback",
        joint_plan_was_built=False,
        joint_plan_was_scored=False,
        joint_plan_was_selected=False,
        local_plan_was_built=True,
        local_plan_was_scored=True,
        local_plan_was_selected=True,
    )
    ...
    # For manual Local publication, the target-planner service remains configured for
    # Joint target preplanning. Leaving it active can pull an unrelated candidate for
    # the same slot before the already-published Local plan is consumed. Disable that
    # service path after formal store/cache publication; materialization and execution
    # still consume the requested Local plan through normal store/publisher boundaries.
...
def _worker(
    ...
    try:
        ...
        expected_phase_independent = _is_phase_independent_policy(requested_policy_id)
...
def run_gate_with_backend(
    *,
    ...
    policy_name: str = "future:p012:joint:global:rscf",
    matrix_bundle_path: str = "",
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--policy-name", default=str(os.environ.get("RS_M123_GATE_POLICY_NAME", "future:p012:joint:global:rscf") or "future:p012:joint:global:rscf"))
