...
def _runtime(rank: int, local_rank: int, *, policy_name: str, p2_hint_mode: str, online_p2_predictor: str) -> RouterSenseInjectionRuntime:
    return RouterSenseInjectionRuntime(
        config=RouterSenseInjectionConfig(
            policy=policy_name,
            planner_id="current:p012:joint:event:rscf" if policy_name == "prepared_priority" else "",
...
def main() -> None:
    ...
    try:
        ...
        strategy_specs = [
            ("fifo_async_p2p", "fifo_bucket", "none", "none", (1,), 2),
            ("greedy_async_p2p", "greedy_bucket", "none", "none", (1,), 2),
            ("birkhoff_phase_local_async_p2p", "birkhoff_bucket_phase_local", "none", "none", (1,), 2),
            ("routersense_joint_zero_hint_async_p2p", "prepared_priority", "none", "none", (1,), 2),
            ("routersense_joint_predicted_async_p2p", "prepared_priority", "calibrated_artifact", "copy_current_dispatch", (1, 2), 2),
        ]
        selected_strategies = {
            item.strip()
            ...
            strategy_summaries.append(
                {
                    ...
                    "secondary_policy_invocation_count": int(plan_summary.get("secondary_policy_invocation_count", 0) or 0),
