...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--strategy", default="routersense_future_p012_joint_event_rscf_async")
...
def main() -> int:
    ...
    payload.update(
        {
            ...
            "local_fallback_build_count": int(rank0_summary.get("local_fallback_build_count", 0) or 0),
    ...
    payload.update(
        {
            ...
            "joint_candidate_makespan": latest_plan_event.get("ideal_joint_candidate_makespan", rank0_summary.get("ideal_joint_candidate_makespan")),
            "local_fallback_makespan": latest_plan_event.get("ideal_local_fallback_makespan", rank0_summary.get("ideal_local_fallback_makespan")),
            "host_projected_joint_candidate_makespan": latest_plan_event.get("host_projected_joint_candidate_makespan", rank0_summary.get("host_projected_joint_candidate_makespan")),
            "host_projected_local_fallback_makespan": latest_plan_event.get("host_projected_local_fallback_makespan", rank0_summary.get("host_projected_local_fallback_makespan")),
    ...
    is_current_local = strategy_name == "routersense_current_p012_local_event_rscf_async"
    is_current_joint = strategy_name == "routersense_current_p012_joint_event_rscf_async"
    is_u_predicted = is_async and p2_hint_mode != "none"
    is_safe = is_u_predicted and safe_projection_mode == "host_select"
    is_async_baseline = is_async and not is_current_joint and not is_u_predicted and not is_current_local
    checks = {
    ...
    if is_native:
    ...
    elif is_current_local or is_async_baseline:
        ...
        checks["real_send_recv_present"] = int(payload["real_send_op_count"]) > 0 and int(payload["real_recv_op_count"]) > 0
        if is_safe:
            checks["safe_local_fallback_built"] = int(payload.get("local_fallback_build_count", 0) or 0) > 0
            ...
            checks["raw_no_local_fallback_build"] = int(payload.get("local_fallback_build_count", 0) or 0) == 0
