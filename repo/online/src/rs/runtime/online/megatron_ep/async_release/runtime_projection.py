...
estimating runtime-safe raw-U vs paired-B behavior.
"""
...
def host_project_safe_selection(
    *,
    raw_u_plan: LogicalSchedulePlan,
    paired_b_plan: LogicalSchedulePlan,
) -> dict[str, Any]:
    projector = RuntimeHostFeasibilityProjector()
    raw_proj = projector.project(raw_u_plan)
    b_proj = projector.project(paired_b_plan)
    select_b = float(raw_proj.host_projected_estimated_makespan) > float(b_proj.host_projected_estimated_makespan)
    return {
        "ideal_raw_u_estimated_makespan": float(raw_proj.ideal_estimated_makespan),
        "host_projected_raw_u_estimated_makespan": float(raw_proj.host_projected_estimated_makespan),
        "ideal_paired_b_estimated_makespan": float(b_proj.ideal_estimated_makespan),
        "host_projected_paired_b_estimated_makespan": float(b_proj.host_projected_estimated_makespan),
        "host_projected_safe_selection": str(paired_b_plan.policy_name if select_b else raw_u_plan.policy_name),
