"""Priority artifact builders for guarded paired U-family plans."""
...
class PairedUPriorityArtifact:
    ...
    def priority_digest(self) -> str:
        return _stable_digest(
            {
                "source_safe_policy": self.source_safe_policy,
                "selected_policy": self.selected_policy,
                "fallback_to_paired_b": self.fallback_to_paired_b,
...
def build_priority_artifact_from_plan(
    *,
    ...
    return PairedUPriorityArtifact(
        source_safe_policy=str(plan.policy_name),
        raw_u_policy=str(plan.diagnostics.get("raw_u_policy", "")),
        paired_b_policy=str(plan.diagnostics.get("paired_b_policy", "")),
...
__all__ = ["PairedUPriorityArtifact", "PriorityEntry", "build_priority_artifact_from_plan"]
