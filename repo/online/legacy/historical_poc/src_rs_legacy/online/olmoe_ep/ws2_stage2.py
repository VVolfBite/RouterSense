...
from ..distributed_runtime import collective_device_for_backend
...
def _all_gather_hash_matrix(local_hashes: list[str], *, device: torch.device) -> list[list[str]]:
    local_tensor = torch.stack([_hash_to_tensor(value, device=device) for value in local_hashes], dim=0)
...
def _route_metadata_int_tensor(routes: list[OnlineRouteRecord]) -> torch.Tensor:
    if not routes:
        return torch.empty((0, 12), dtype=torch.int64)
    rows = [
        [
...
def _decode_received_routes(
    *,
    ...
    for row_index in range(int(metadata_int.shape[0])):
        ...
        request_numeric_id = int(row[0].item())
        microbatch_numeric_id = int(row[1].item())
        layer_id = int(row[2].item())
        source_rank = int(row[3].item())
        destination_rank = int(row[4].item())
        source_node_id = int(row[5].item())
        destination_node_id = int(row[6].item())
        local_token_index = int(row[7].item())
        topk_slot = int(row[8].item())
        expert_id = int(row[9].item())
        payload_rows = int(row[10].item())
        payload_bytes = int(row[11].item())
        received.append(
            OnlineRouteRecord(
...
def execute_ws2_hidden_dispatch_only(
    *,
    ...
    collective_device = collective_device_for_backend(backend, hidden_states.device)
    ordered_routes = _ordered_remote_routes(partition)
    pack_started = time.perf_counter()
    send_hidden = _build_send_hidden_tensor(hidden_states, ordered_routes).to(device=collective_device)
    send_metadata_int = _route_metadata_int_tensor(ordered_routes).to(device=collective_device)
    send_metadata_weight = _route_metadata_weight_tensor(ordered_routes).to(device=collective_device)
    ...
    gathered_hidden_digests = _all_gather_hash_matrix(local_hidden_digests, device=collective_device)
    gathered_route_digests = _all_gather_hash_matrix(local_route_digests, device=collective_device)
    ...
    try:
        ...
        transport_record = TransportOperationRecord(
            ...
            local_rank=None if hidden_states.device.index is None else int(hidden_states.device.index),
