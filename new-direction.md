...
**验证方法**: 跑一次forward pass，比较相邻层gate输出的cosine similarity和expert选择top-K hit rate。
...
第二个特征是跨层流量的可预测性。已有工作（Fate ASPLOS'25）在多个MoE模型上验证了相邻层的专家选择具有极高的重叠度，据此可实现97%以上的专家选择预测准确率（非基于gate输入相似度的间接推断，而是在离散top-K选择层面的直接度量）。然而，现有工作仅将这一跨层相关性用于expert prefetching（Fate、Pre-gated MoE）和expert placement（s-MoE、ExFlow），从未将其用于通信调度决策。这一空白并非因为调度层面的价值被否定，而是因为场景成熟度的时序问题：跨层预测的准确率直到近期才被验证到可用水平（>97%），在此之前的预测误差对于调度决策（直接影响实时延迟）的风险远高于对于prefetching（仅影响命中率）的风险；当预测准确率跨过阈值后，调度层面的应用窗口才真正打开。
