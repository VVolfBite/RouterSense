...
from rs.planning.runtime_bridge import to_scheduling_request
...
def test_information_mode_filters_legacy_phase_rows(
    ...
    scheduling = to_scheduling_request(request)
    assert scheduling.p2_hint_rows == expected_p2
    assert scheduling.scheduling_mode == ("runtime_lookahead" if track == "runtime_lookahead" else "execution_window")
