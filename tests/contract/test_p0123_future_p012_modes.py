...
def test_p0123_derives_transposed_p3_but_executes_only_current_p0_p1() -> None:
    ...
    plan = PlannerRegistry.create("barrier_criticality_joint", usage="runtime").plan(request)
    phases = {flow.phase for wave in plan.waves for flow in wave.flows}
    assert phases <= {"p0_dispatch", "p1_return"}
    assert "p2_next_dispatch_forecast" not in phases
    assert "p3_next_return_forecast" not in phases


def test_p0123_weight_zero_is_execution_order_equivalent_to_frozen_p012() -> None:
    planner = PlannerRegistry.create("barrier_criticality_joint", usage="runtime")
...
def test_p0123_return_advisory_can_change_current_p0_p1_plan() -> None:
    ...
    assert float(p0123.metadata["makespan"]) >= 0.0
...
def _runtime(*, timing: str, horizon: str = "p012") -> RouterSenseInjectionRuntime:
    planner_id = (
        "future_prepared:global:rscf"
        if timing == "previous_layer"
        else "p0123:global:rscf"
        if horizon == "p0123"
        else "p012:global:rscf"
    )
...
def _future_request() -> TargetLayerPlanningRequest:
    return TargetLayerPlanningRequest(
        ...
        policy_id="future_prepared:global:rscf",
        raw_u_policy_id="future_prepared:global:rscf",
        paired_b_policy_id="p012:local:rscf",
        safe_projection_mode="disabled",
        group_size=3,
        bucket_rows=0,
...
def test_future_p012_reuses_exact_on_demand_p012_planner_output() -> None:
    ...
    on_demand = PlannerRegistry.create("p012:global:rscf", usage="runtime").plan(built.planning_request)
...
def test_public_strategy_presets_keep_three_modes_distinct() -> None:
    p012 = resolve_strategy_runtime(strategy_name="routersense_p012_async", runtime_line="async_release")
    p0123 = resolve_strategy_runtime(strategy_name="routersense_p0123_async", runtime_line="async_release")
    future = resolve_strategy_runtime(strategy_name="routersense_future_p012_async", runtime_line="async_release")
    ...
    assert p012["planner_id"] == "p012:global:rscf"
    assert p0123["planner_id"] == "p0123:global:rscf"
    assert future["planner_id"] == "future_prepared:global:rscf"


@pytest.mark.parametrize("core", ["gmwd", "rsbc", "rscf"])
@pytest.mark.parametrize("branch", ["local", "event", "global"])
def test_public_p012_strategy_exposes_all_three_cores_and_branches(core: str, branch: str) -> None:
    strategy = resolve_strategy_runtime(
        strategy_name=f"routersense_p012_{branch}_{core}_async",
        runtime_line="async_release",
    )
    assert strategy["planner_id"] == f"p012:{branch}:{core}"
