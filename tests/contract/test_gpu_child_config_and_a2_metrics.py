...
def test_synthetic_hotpath_aggregate_uses_all_ranks() -> None:
    ...
    assert aggregate["joint_build_count_all_rank_sum"] == 16
    assert aggregate["expected_joint_build_upper_bound_all_rank"] == 16
    assert aggregate["joint_build_count_all_rank_valid"] is True
    assert aggregate["joint_build_count_by_layer_per_rank_valid"] is True
    ...
    ranks = [
        {
            ...
            "shadow_control_collective_count": 0,
            "joint_build_count": 4,
            "joint_build_count_by_layer_per_rank": {"0": 2, "1": 2},
            "local_build_count": 0,
            "predict_count": 0,
        }
        for rank in range(4)
...
def test_synthetic_hotpath_joint_candidate_layer_upper_bound_is_ineligible() -> None:
    ranks = [
        {
            "rank": rank,
            "selected_p0_hook_count": 4,
            ...
            "shadow_plan_build_count": 0,
            "shadow_control_collective_count": 0,
            "joint_build_count": 5 if rank == 0 else 4,
            "joint_build_count_by_layer_per_rank": {"0": 3, "1": 2} if rank == 0 else {"0": 2, "1": 2},
            "local_build_count": 0,
            "predict_count": 0,
        }
        for rank in range(4)
    ...
    assert aggregate["hotpath_eligible"] is False
    assert aggregate["joint_build_count_per_rank_valid"] is False
    assert aggregate["joint_build_count_by_layer_per_rank_valid"] is False
    assert any("joint_build_count_by_layer:rank=0:layer=0:upper=2:actual=3" in item for item in aggregate["eligibility_reasons"])
