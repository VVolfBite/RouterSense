...
def test_imperfect_prediction_remains_valid_when_execution_truth_differs() -> None:
    ...
    policy_name = resolve_algorithm_id("islip_reference").builder_key
    for hint in (
        ...
        assert result["planning_track"] == "runtime_lookahead"
        assert result["p2_semantics"] == ("absent" if hint.hint_type == "zero_hint" else "advisory_hint")
        assert result["logical_plan_digest"] == result["forecast_plan_digest"]
        assert result["execution_bound_plan_digest"]
...
def test_formal_joint_zero_hint_binds_actual_p2_before_execution_audit() -> None:
    window = _window()
    engine = ReplayEngine(scheduling_mode="execution_window", expert_compute_delay=0.0, bucket_rows=2)
    result = engine.execute(
        replay_window=window,
        planning_hint=PlanningHint("zero_hint", ((0, 0), (0, 0)), 0.0, None, 2),
        policy_name="current:p012:joint:event:rscf",
    )
    assert result["audit_valid"] is True
    assert result["audit"]["target_volume_by_phase"][2] == 5.0
    assert result["audit"]["served_volume_by_phase"][2] == 5.0


def test_terminal_zero_truth_discards_nonzero_advisory_p2_before_audit() -> None:
    window = ReplayWindow(
        fixture_id="fixture",
        window_id="terminal",
        layer_id=1,
        p0_truth_rows=((0, 3), (2, 0)),
        p1_truth_rows=((0, 2), (3, 0)),
        p2_truth_rows=((0, 0), (0, 0)),
        matrix_unit="rows",
        group_size=2,
        payload_row_bytes_by_phase={"P0": 1, "P1": 1, "P2": 1},
        metadata={"terminal_layer": True},
    )
    result = ReplayEngine(
        ...
        replay_window=window,
        planning_hint=PlanningHint("history_ema", ((0, 4), (1, 0)), 0.75, 1, 2),
        policy_name="current:p012:joint:event:rscf",
    )
    assert result["audit_valid"] is True
    assert result["audit"]["target_volume_by_phase"][2] == 0.0
    assert result["audit"]["served_volume_by_phase"][2] == 0.0
    assert result["execution_bound_plan_digest"] != result["forecast_plan_digest"]
...
def test_public_policy_catalog_converges_birkhoff_and_reference_split() -> None:
    ...
    assert birkhoff.internal_policy_name == "birkhoff_bucket_phase_local"
...
def test_legacy_and_canonical_algorithm_names_produce_identical_plan_digest() -> None:
    ...
    engine = ReplayEngine(scheduling_mode="execution_window", expert_compute_delay=0.0, bucket_rows=2)
    legacy = resolve_algorithm_id("birkhoff_bucket_phase_local")
