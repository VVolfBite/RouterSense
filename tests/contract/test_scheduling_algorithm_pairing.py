...
def test_b_birkhoff_is_phase_local_oracle_like_reference() -> None:
    ...
    assert "same canonical bucket/wave exact model" in local["notes"]
...
def test_joint_oracle_and_legacy_granularity_are_classified_explicitly() -> None:
    ...
    assert get_algorithm_metadata("O_local_phase_oracle")["granularity_mode"] == "dynamic_bucket_current"
