...
def test_runtime_presets_distinguish_raw_and_safe_async_joint_paths() -> None:
    predicted_raw = resolve_strategy_runtime(
        strategy_name="routersense_future_p012_joint_event_rscf_async",
...
def test_runtime_raw_and_safe_scope_store_different_selected_plan_digests(monkeypatch) -> None:
    matrix = ((0, 5), (3, 0))
    contexts = make_contexts_from_matrix(phase="P0", matrix=matrix, p2_hint_mode="none")

    def _select_paired(*, joint_plan, local_plan):
        return {
            "ideal_joint_candidate_estimated_makespan": 10.0,
            "host_projected_joint_candidate_estimated_makespan": 10.0,
            "ideal_local_fallback_estimated_makespan": 8.0,
            "host_projected_local_fallback_estimated_makespan": 8.0,
            "host_projected_safe_selection": str(local_plan.policy_name),
        }
    ...
    joint_candidate_plan = raw_runtime._runtime_state.read("global_joint_window_plan")  # noqa: SLF001
    safe_plan = safe_runtime._runtime_state.read("global_joint_window_plan")  # noqa: SLF001
    assert joint_candidate_plan["safe_projection_mode"] == "disabled"
    assert safe_plan["safe_projection_mode"] == "host_select"
    assert joint_candidate_plan["safe_selected_policy"] != safe_plan["safe_selected_policy"]
...
def test_runtime_raw_and_safe_scope_can_naturally_diverge_without_monkeypatch() -> None:
    ...
    joint_candidate_plan = raw_runtime._runtime_state.read("global_joint_window_plan")  # noqa: SLF001
    safe_plan = safe_runtime._runtime_state.read("global_joint_window_plan")  # noqa: SLF001
    assert joint_candidate_plan["safe_selected_policy"] == "current:p012:joint:global:rscf"
    assert safe_plan["local_planner_name"] == "current:p012:local:global:rscf"
    assert safe_plan["safe_selected_policy"] in {
        "current:p012:joint:global:rscf",
        "current:p012:local:global:rscf",
    }
    assert joint_candidate_plan["local_build_count"] == 0
    assert safe_plan["local_build_count"] > 0
    assert joint_candidate_plan["host_projection_count"] == 0
