...
def test_bucketed_fifo_and_reverse_share_same_transfer_layout() -> None:
    ...
    fifo = resolve_policy(policy_name="fifo_bucket", bucket_rows=16).build_plan(
        local_context=ctx0,
        global_contexts=global_contexts,
    )
    reverse = resolve_policy(policy_name="reverse_bucket_reference", bucket_rows=16).build_plan(
        local_context=ctx0,
        global_contexts=global_contexts,
    )
