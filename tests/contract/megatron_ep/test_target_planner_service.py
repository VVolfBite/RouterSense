...
def test_raw_target_planner_does_not_build_local_fallback() -> None:
    ...
    assert plan.selected_variant == "joint_candidate"
    assert plan.local_fallback_logical_plan_digest == ""
    assert plan.safe_projection_mode == "disabled"
    assert plan.local_plan_was_built is False
    assert plan.local_plan_was_scored is False
    assert plan.safe_selection_us == 0.0
    assert plan.local_build_us == 0.0


def test_safe_target_planner_builds_local_fallback_and_records_selection() -> None:
    service = TargetLayerPlannerService(store=TargetPlanStore())
    built = service._build_target_plan(  # noqa: SLF001
    ...
    plan = built.prepared_plan
    assert plan.joint_candidate_logical_plan_digest != ""
    assert plan.local_fallback_logical_plan_digest != ""
    assert plan.selected_logical_plan_digest != ""
    assert plan.safe_projection_mode == "host_select"
    assert plan.local_plan_was_built is True
    assert plan.local_plan_was_scored is True
    assert plan.selected_variant in {"joint_candidate", "local_fallback"}
    assert plan.local_build_us >= 0.0
    assert plan.safe_selection_us >= 0.0
...
class _CountingPlanner:
    ...
    def plan(self, request: PlanningRequest) -> WindowPlan:
        ...
        return WindowPlan(
            ...
            metadata={"policy_name": self.planner_id},
        )
...
def test_target_planner_service_core_selection_does_not_replan() -> None:
    ...
    def planner_factory(planner_id: str, _config) -> _CountingPlanner:
        family = "joint" if ":joint:" in planner_id or "joint" in planner_id else "local"
    ...
    joint_planner = planner_factory("future:p012:joint:global:rscf", None)
    ...
    assert counter.get("future:p012:joint:global:rscf", 0) == 0
    counter.clear()
    ...
    assert compare_selector.selected_plan.planner_id == "future:p012:joint:global:rscf"
    assert counter.get("fifo_bucket", 0) == 0
    assert counter["future:p012:joint:global:rscf"] == 1
    counter.clear()
    local_plan = local_planner.plan(formal_request)
    joint_plan = joint_planner.plan(formal_request)
    compare_selector = service._select_candidate_plans(  # noqa: SLF001
...
def test_target_planner_build_path_counts_planner_calls_once_per_mode() -> None:
    ...
    assert counter["future:p012:joint:global:rscf"] == 1
    assert counter.get("future:p012:local:global:rscf", 0) == 0
    counter.clear()
    service._build_target_plan(  # noqa: SLF001
        request=_request(safe_projection_mode="host_select"),
        metrics=TargetLayerPlannerMetrics(),
    ...
    assert counter["future:p012:joint:global:rscf"] == 1
    assert counter["future:p012:local:global:rscf"] == 1
...
def test_sync_and_preplanned_formal_plan_digests_match_for_same_planning_request() -> None:
    ...
    runtime_plan = PlannerRegistry.create("future:p012:joint:global:rscf", None).plan(runtime_request)
    service_plan = PlannerRegistry.create("future:p012:joint:global:rscf", None).plan(service_request)
