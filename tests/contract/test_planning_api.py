...
def test_planner_registry_exposes_formal_metadata() -> None:
    ...
    assert specs["islip_reference"].planner_family == "reference_local"
    assert specs["islip_reference"].reference_only is True
    assert specs["birkhoff_fluid_reference"].planner_family == "reference_local"
    assert specs["oracle_local_exact"].planner_family == "exact_local"
    assert specs["oracle_local_exact"].exact is True
    assert specs["oracle_joint_exact"].planner_family == "exact_joint"
    assert specs["future:p012:joint:global:rscf"].planner_family == "joint"
    assert all(not spec.historical_aliases for spec in specs.values())
...
def test_window_plan_semantic_digest_excludes_metadata_but_audit_digest_changes() -> None:
    ...
    same_waves_new_metadata = type(plan)(
        ...
        waves=plan.waves,
        metadata={"communication_makespan": 123.0, "note": "different"},
    )
