...
def test_axes_parser_rejects_removed_legacy_ids() -> None:
    for planner_id in ("p012:local:rscf", "p012:global:rscf", "future_prepared:event:rsbc"):
        with pytest.raises(ValueError, match="formal planner ID"):
            parse_planner_axes(planner_id)
    assert parse_planner_axes("current:p012:local:global:gmwd").canonical_id == "current:p012:local:global:gmwd"
...
def test_explicit_current_axes_are_deterministic(core: str) -> None:
    request = _request()
    for planner_id in (
        f"current:p012:local:event:{core}",
        f"current:p012:joint:event:{core}",
        f"current:p012:joint:global:{core}",
    ):
        left = PlannerRegistry.create(planner_id, usage="runtime").plan(request)
        right = PlannerRegistry.create(planner_id, usage="runtime").plan(request)
        assert _flow_signature(left) == _flow_signature(right)
        assert left.metadata["planner_axes"] == parse_planner_axes(planner_id).to_dict()
...
def test_runtime_safe_pair_changes_only_scope() -> None:
    ...
    assert runtime._runtime_safe_scope_pair(planner_id) == (  # noqa: SLF001
        planner_id,
