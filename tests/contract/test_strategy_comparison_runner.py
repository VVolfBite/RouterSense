...
def test_formal_future_strategy_preserves_planner_and_prediction_config(tmp_path: Path) -> None:
    config_path = tmp_path / "comparison.yaml"
    config_path.write_text(
        yaml.safe_dump(
            {
                ...
                "workload": {"prompts": "configs/workload/smoke_prompts.json"},
                "prediction": {
                    "name": "fate_cross_layer_gate",
                    "config": {"second_hop_predictor_id": "bridge_copy_current"},
                },
                "strategies": [{"name": "routersense_future_p012_global_rsbc_async"}],
                "execution": {"repetitions": 1},
            },
            sort_keys=False,
        ),
    ...
    generated = yaml.safe_load(
        (output_dir / "generated_configs" / "routersense_future_p012_global_rsbc_async_rep0.yaml").read_text(
            encoding="utf-8"
        )
    )
    parameters = generated["online_policy"]["parameters"]
    assert generated["online_policy"]["name"] == "routersense_p0p1p2_hint"
    assert parameters["planner_id"] == "future_prepared:global:rsbc"
    assert parameters["planning_horizon"] == "p012"
    assert parameters["planning_timing"] == "previous_layer"
    assert parameters["online_p2_predictor"] == "fate_cross_layer_gate"
    assert parameters["online_p2_predictor_config"] == {
        "second_hop_predictor_id": "bridge_copy_current"
    }
