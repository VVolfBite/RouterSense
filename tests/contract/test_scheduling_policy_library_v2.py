...
from rs.reference.baselines.islip_round_robin import ISLIPNoProgressError, ISLIPRoundRobinPolicy, _schedule_flows
...
def test_islip_round_robin_coverage_and_determinism(fixture: str) -> None:
    ...
    policy = resolve_policy(policy_name="islip_reference", bucket_rows=16)
...
def test_exact_reference_detects_nonoptimal_heuristic_and_optimal_policy() -> None:
    ...
    greedy = resolve_policy(policy_name="greedy_bucket", bucket_rows=16).build_logical_plan(problem)
    ...
    assert exact["objective_logical_makespan"] == exact["best_bound"]


def test_policy_matrix_consistency() -> None:
    assert "islip_reference" not in SUPPORTED_ONLINE_PHASE_POLICIES
    assert "power_of_two_reference" not in SUPPORTED_ONLINE_PHASE_POLICIES
    assert not resolve_policy(policy_name="islip_reference", bucket_rows=16).capabilities.supports_online_phase_local_execution
    assert not resolve_policy(policy_name="birkhoff_fluid_reference", bucket_rows=16).capabilities.supports_online_phase_local_execution
    with pytest.raises(Exception):
        resolve_phase_policy(policy_name="birkhoff_fluid_reference", bucket_rows=16)
    with pytest.raises(Exception):
        resolve_phase_policy(policy_name="oracle_local_exact", bucket_rows=16)
    with pytest.raises(Exception):
        resolve_phase_policy(policy_name="future:p012:joint:global:rscf", bucket_rows=16)
...
@pytest.mark.parametrize("fixture", ["phase_barrier_4rank", "receiver_incast_4rank", "full_duplex_pair_4rank", "skewed_8rank"])
def test_related_work_style_cores_cover_replay_flows(policy_name: str, fixture: str) -> None:
    problem = _problem(fixture)
    policy = resolve_policy(policy_name=policy_name, bucket_rows=16)
    plan = policy.build_logical_plan(problem)
    validation = validate_logical_plan(plan, expected_flows=_expected_flows(problem))
    assert validation["valid"], validation["errors"]
    assert plan.diagnostics["literature_mapping"] == "style"


def test_islip_pointers_persist_across_waves() -> None:
    flows = (
        FlowDemand("a", "p0_dispatch", 0, 1, 10, "ready", True),
        FlowDemand("b", "p0_dispatch", 0, 2, 8, "ready", True),
        FlowDemand("c", "p0_dispatch", 1, 2, 7, "ready", True),
        FlowDemand("d", "p0_dispatch", 2, 0, 6, "ready", True),
    )
    _, trace = _schedule_flows(
        flows,
        ranks=(0, 1, 2),
        seed_payload={"case": "persistent-pointers"},
        start_wave_id=0,
        max_rounds=2,
    )
    assert len(trace) >= 2
    assert trace[1]["input_pointer_before"] == trace[0]["input_pointer_after"]
    assert trace[1]["output_pointer_before"] == trace[0]["output_pointer_after"]
