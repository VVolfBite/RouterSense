...
def _fully_covered_request_2x2() -> PlanningRequest:
    return PlanningRequest(
        identity=PlanningIdentity(request_id="req"),
        traffic=PlanningTraffic(p0_dispatch_rows=((0, 3), (2, 0)), p1_return_rows=((0, 1), (2, 0))),
...
def _fully_covered_plan_2x2(request: PlanningRequest, *, wave0_duration: float = 0.0) -> WindowPlan:
    return WindowPlan(
        planner_id="test",
        ...
        waves=(
            PlanWave(
                wave_id=0,
                flows=(
                    PlannedFlow(flow_id="p0", phase="p0_dispatch", src_rank=0, dst_rank=1, row_count=3, release_state="ready", executable=True),
                    PlannedFlow(flow_id="p1", phase="p1_return", src_rank=1, dst_rank=0, row_count=2, release_state="ready", executable=True),
                ),
                estimated_duration=wave0_duration,
            ),
            PlanWave(
                wave_id=1,
                flows=(
                    PlannedFlow(flow_id="p0b", phase="p0_dispatch", src_rank=1, dst_rank=0, row_count=2, release_state="ready", executable=True),
                    PlannedFlow(flow_id="p1b", phase="p1_return", src_rank=0, dst_rank=1, row_count=1, release_state="ready", executable=True),
                ),
...
def test_plan_estimator_ignores_legacy_makespan_metadata_for_decision_value() -> None:
    ...
    base_plan = WindowPlan(
        ...
        planner_family="baseline",
        request_digest=request.semantic_digest(),
        waves=_fully_covered_plan_2x2(request).waves,
    ...
    assert first.estimated_makespan == second.estimated_makespan == 12.0


def test_plan_estimator_ignores_legacy_wave_duration_for_decision_value() -> None:
    request = _fully_covered_request_2x2()
    fast = _fully_covered_plan_2x2(request, wave0_duration=0.0)
    slow_legacy = WindowPlan(
        planner_id="test",
        planner_family="baseline",
        request_digest=request.semantic_digest(),
        waves=(
            PlanWave(wave_id=0, flows=fast.waves[0].flows, estimated_duration=999.0),
            fast.waves[1],
        ),
    )
    estimator = CommonCorePlanEstimator()
...
def test_plan_estimator_allows_full_duplex_send_and_receive_same_rank() -> None:
    request = PlanningRequest(
        identity=PlanningIdentity(request_id="req"),
        traffic=PlanningTraffic(p0_dispatch_rows=((0, 1, 0), (0, 0, 0), (1, 0, 0)), p1_return_rows=((0, 0, 0), (0, 0, 0), (0, 0, 0))),
