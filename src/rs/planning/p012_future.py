...
def _to_formal_plan(
    ...
    *,
    ...
    family_id = str(metadata.get("family", ""))
    if family_id:
        metadata["common_core"] = get_family_kernel_spec(family_id).contract()
    formal = WindowPlan(
...
# The formal registry exposes only explicit orthogonal planner IDs.
...
def p012_planner_specs() -> tuple[PlannerSpec, ...]:
    """Return the complete explicit orthogonal planner matrix."""
    return _EXPLICIT_AXIS_SPECS
