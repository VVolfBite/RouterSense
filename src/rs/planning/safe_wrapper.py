...
def _infer_core_key(planner_id: str) -> str | None:
    normalized = str(planner_id).lower()
    parts = normalized.split(":")
    if len(parts) == 3 and parts[0] in {"p012", "p0123", "future_prepared"}:
        return parts[2]
    aliases = (
        ("rscf", "rscf"), ("rsbc", "rsbc"), ("gmwd", "gmwd"),
        ("gated_greedy", "gated_greedy"),
        ("gated_maxweight", "gated_maxweight"),
        ("barrier_criticality", "barrier_criticality"),
        ("barrier_price", "barrier_price"),
        ("lagrangian", "lagrangian"),
        ("ibbr", "birkhoff"), ("birkhoff", "birkhoff"),
    )
        ...
        metadata.update(
            {
                ...
                "fallback_to_paired_b": selected.role == "local",
