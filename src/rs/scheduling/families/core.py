...
class FamilyKernelSpec:
    ...
    # Canonical four-component runtime vector consumed by the migrated P012
    # event/global kernels: residual, critical-path, transitive-unlock, endpoint.
    # Keeping it on the shared FamilyKernelSpec removes the second family-spec
    # authority that previously drifted under p012_future/_kernel/families.py.
    ...
    def p012_runtime_weights(self) -> tuple[float, float, float, float]:
        if self.p012_weights is None:
            raise ValueError(f"family {self.family_id!r} has no P012 runtime weights")
        values = tuple(float(value) for value in self.p012_weights)
        if len(values) != 4:
            raise ValueError(f"family {self.family_id!r} P012 runtime weights must have length four")
        return values
