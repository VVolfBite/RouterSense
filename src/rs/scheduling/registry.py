...
from .multiphase.safe_pair import SafePlannerWrapper
...
def resolve_policy(
    *,
    ...
    requested_policy_name = str(policy_name)
    base_name, mode = _parse_policy_name(policy_name)
    legacy_safe_names = {
        "RS_safe_gated_greedy", "RS_safe_gated_maxweight",
        "RS_safe_barrier_criticality", "RS_safe_rscf",
        "RS_safe_barrier_price", "RS_safe_lagrangian", "RS_safe_ibbr",
    }
    safe_lookup_name = requested_policy_name if requested_policy_name in legacy_safe_names else base_name
    ...
    if safe_lookup_name in {
        ...
        raw_u_name, paired_b_name = {
            ...
            "RS_safe_rscf": ("rscf_joint", "rscf_local"),
            ...
            "safe_pair_selector": ("rscf_joint", "rscf_local"),
            "safe_pair": ("rscf_joint", "rscf_local"),
        }[safe_lookup_name]
        ...
        return SafePlannerWrapper(
            policy_name=safe_lookup_name,
            joint_policy_name=raw_u_name,
            local_policy_name=paired_b_name,
...
def resolve_phase_policy(
    *,
    ...
    if policy_name == "bucketed_fifo":
        return BucketedFIFOPolicy(bucket_rows=bucket_rows)
    # Preserve the historical phase-local online contract for the deprecated
    # alias. The canonical aurora_order_local identifier uses the paired-family
    # formal planner, while old runtime configs still expect build_plan().
