...
def request_to_problem(request: SchedulingRequest) -> MultiPhaseSchedulingProblem:
...
class LogicalPolicyAdapter:
    ...
    def plan(self, request: SchedulingRequest) -> LogicalSchedulePlan:
        problem = request_to_problem(request)
...
def build_policy(policy_id: str, options: PolicyOptions) -> SchedulingPolicy:
    ...
    return LogicalPolicyAdapter(
        canonical_policy_id=str(resolved.canonical_name),
        builder_key=str(resolved.builder_key),
        options=options,
        capabilities=getattr(policy, "capabilities", PolicyCapabilities(
