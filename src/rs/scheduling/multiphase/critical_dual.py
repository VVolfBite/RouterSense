...
def release_critical_dual_candidates(
    *,
    ...
    return_advisory_matrix: list[list[int]] | None = None,
    return_advisory_weight: float = 0.0,
    ...
    if return_advisory_matrix is not None and float(return_advisory_weight) > 0.0:
        confidence = max(0.0, min(1.0, float(future_confidence)))
        for src, row in enumerate(return_advisory_matrix):
            for dst, value in enumerate(row):
                if src == dst or float(value) <= 0.0:
                    continue
                edge[(3, src, dst)] = confidence * float(return_advisory_weight) * float(value)
    ...
    p3_tail: dict[int, float] = {rank: outgoing.get((3, rank), 0.0) for rank in ranks}

    # A P2 source tail contains its serial send work, destination exposure,
    # and the return rows released at the P2 destinations. P3 remains
    # advisory geometry only and never becomes executable work.
    ...
    for rank in ranks:
        ...
        reachable_return_tail = max(
            [p3_tail.get(dst, 0.0) for dst, volume in phase_rows.get((2, rank), ()) if volume > 1e-9],
            default=0.0,
        )
        p2_tail[rank] = row_volume + (
            float(destination_hotspot_weight) * hotspot_exposure
        ) + reachable_return_tail
    ...
    max_tail = max([1.0, *p1_tail.values(), *p2_tail.values(), *p3_tail.values()])
