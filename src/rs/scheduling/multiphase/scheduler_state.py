...
def run_global_matching_scheduler(
    ...
    *,
    ...
    if return_advisory_matrix is not None and float(return_advisory_weight) > 0.0:
        p3_outgoing = [
            float(sum(int(value) for dst, value in enumerate(row) if int(dst) != int(src)))
            for src, row in enumerate(return_advisory_matrix)
        ]
        for src, row in enumerate(planning_prediction_matrix):
            reachable_return_tail = max(
                [p3_outgoing[dst] for dst, value in enumerate(row) if int(dst) != int(src) and float(value) > 0.0],
                default=0.0,
            )
            downstream_load[(2, src)] = float(downstream_load.get((2, src), 0.0)) + (
                float(return_advisory_weight) * reachable_return_tail
            )
        ...
        if scoring_model == "critical_frontier":
            ready = critical_frontier_candidates(
                **candidate_kwargs,
                ...
                return_advisory_matrix=return_advisory_matrix,
