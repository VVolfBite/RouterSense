...
class BirkhoffPhaseLocalPolicy:
    ...
    def _logical_decompose(self, matrix: tuple[tuple[int, ...], ...], *, phase: str, start_wave_id: int) -> tuple[list[LogicalWave], list[WaveDiagnostics]]:
        ...
        while residual:
            ...
            if not chosen:
                raise ValueError("birkhoff_bucket_phase_local could not find a legal matching")
    ...
    def build_plan(
        ...
        *,
        ...
        while any(residual.values()):
            ...
            if not chosen_tasks:
                raise ValueError("birkhoff_bucket_phase_local could not select any bucket task")
