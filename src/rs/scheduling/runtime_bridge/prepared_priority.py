"""Compile an already selected logical plan into current-phase bucket waves.

This is a materialization policy, not a scheduling algorithm. The logical
ordering is produced by the formal Current/Future P012 planner and carried in
``FutureDemandHint.metadata``.
"""
...
from rs.scheduling.phase_local.fifo import build_transfer_layouts_and_tasks, finalize_execution_plan
...
class PreparedPriorityPhasePolicy:
    ...
    def build_plan(self, *, local_context: PhaseReadyContext, global_contexts: tuple[PhaseReadyContext, ...]) -> PhaseExecutionPlan:
        ...
        hint_pressure = {
            int(rank): (_hint_rank_pressure(local_context.p2_hint.hint_digest, int(rank)) if hint_available and self.p2_hint_weight > 0.0 else 0)
            for rank in local_context.ep_group_ranks
        }
        edge_priority = _preferred_edge_priority(local_context.p2_hint.metadata, phase=local_context.phase) if hint_available else {}
        ...
        def task_priority(task) -> tuple:
            ...
            return (-base_score, -int(task.byte_count), int(task.src_rank), int(task.dst_rank), int(task.bucket_ordinal))

        sort_started_ns = time.perf_counter_ns()
        ordered_tasks = sorted(all_tasks, key=task_priority)
        ...
        prediction_used = hint_available and (bool(edge_priority) or self.p2_hint_weight > 0.0)
        diagnostics = {
            "policy_name": self.policy_name,
            "policy_version": self.policy_version,
            "policy_role": "logical_plan_materializer",
            ...
            "uses_prepared_priority": bool(use_prepared_priority),
            "prediction_used": bool(prediction_used),
            "evaluation_eligible": local_context.p2_hint.hint_mode == "calibrated_artifact",
            "prepared_edge_priority": {f"{src}->{dst}": int(priority) for (src, dst), priority in sorted(edge_priority.items(), key=lambda item: item[1])},
...
__all__ = ["PreparedPriorityPhasePolicy"]
