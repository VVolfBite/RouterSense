...
from .asset_registry import (
    create_predictor as create_extended_predictor,
    merge_predictor_specs,
    resolves_predictor as resolves_extended_predictor,
    resolve_predictor as resolve_extended_predictor,
)
...
def resolve_predictor_id(name: str) -> str:
    ...
    if resolves_extended_predictor(normalized):
        return resolve_extended_predictor(normalized)
...
class PredictionRegistry:
    ...
    def specs() -> tuple[PredictorSpec, ...]:
        return merge_predictor_specs(tuple(_SPECS.values()))
    ...
    def create(predictor_id: str, config: Any | None = None, *, usage: str = "runtime") -> Predictor:
        ...
        if resolves_extended_predictor(predictor_id):
            values = config if isinstance(config, dict) else ({} if config is None else vars(config))
            return create_extended_predictor(predictor_id, values)
