...
SUPPORTED_EXECUTION_MODES = {"native_passthrough", "phase_sync_wave", "joint_window_async_p2p"}
SUPPORTED_PLANNING_HORIZONS = {"p012", "p0123"}
SUPPORTED_PLANNING_TIMINGS = {"legacy_auto", "on_demand", "previous_layer"}
...
class OnlinePolicyParameters:
    ...
    planning_horizon: str = "p012"
    planning_timing: str = "legacy_auto"
...
def validate_run_config(config: RunConfig) -> None:
    ...
    if config.online_policy.parameters.planning_horizon not in SUPPORTED_PLANNING_HORIZONS:
        raise ValueError(
            f"unsupported online_policy.parameters.planning_horizon "
            f"{config.online_policy.parameters.planning_horizon!r}"
        )
    if config.online_policy.parameters.planning_timing not in SUPPORTED_PLANNING_TIMINGS:
        raise ValueError(
            f"unsupported online_policy.parameters.planning_timing "
            f"{config.online_policy.parameters.planning_timing!r}"
        )
    if (
        config.online_policy.parameters.planning_horizon == "p012"
        and abs(float(config.online_policy.parameters.p3_return_weight)) > 1e-12
    ):
        raise ValueError("p3_return_weight must be 0 when planning_horizon=p012")
    if (
        config.online_policy.parameters.planning_timing == "previous_layer"
        and config.online_policy.parameters.planning_horizon != "p012"
    ):
        raise ValueError("previous_layer planning currently supports future-p012 only")
    if (
    ...
    if config.run.kind == "online_policy_correctness":
        ...
        if config.execution.mode not in {"phase_sync_wave", "joint_window_async_p2p"}:
            raise ValueError("online_policy_correctness requires phase_sync_wave or joint_window_async_p2p execution")
        ...
        if config.execution.mode == "joint_window_async_p2p" and config.online_policy.name not in {
            "fifo_bucket", "greedy_bucket", "birkhoff_bucket_phase_local", "prepared_priority"
        }:
            raise ValueError("joint_window_async_p2p requires a formal phase baseline or prepared_priority")
        if config.online_policy.name == "prepared_priority" and not str(config.online_policy.parameters.planner_id or ""):
            raise ValueError("prepared_priority requires online_policy.parameters.planner_id")
    ...
    return RunConfig(
        ...
        online_policy=OnlinePolicyConfig(
            ...
            parameters=OnlinePolicyParameters(
                planner_id=_strict_str(
                    online_policy_parameters.get("planner_id"),
                    field_name="online_policy.parameters.planner_id",
                    default="",
                ),
                ...
                p3_return_weight=_strict_float(online_policy_parameters.get("p3_return_weight"), field_name="online_policy.parameters.p3_return_weight", default=0.0, minimum=0.0),
                planning_horizon=_strict_str(online_policy_parameters.get("planning_horizon"), field_name="online_policy.parameters.planning_horizon", default="p012"),
                planning_timing=_strict_str(online_policy_parameters.get("planning_timing"), field_name="online_policy.parameters.planning_timing", default="legacy_auto"),
                ...
                online_p2_predictor_config=_strict_mapping(
                    online_policy_parameters.get("online_p2_predictor_config"),
                    field_name="online_policy.parameters.online_p2_predictor_config",
                ),
            ),
