...
class PlanningWeights:
    ...
    def to_semantic_dict(self) -> dict[str, Any]:
        payload = asdict(self)
        # Preserve the frozen P012 semantic digest.  P3 is an optional
        # advisory extension and must not perturb legacy requests when disabled.
        if abs(float(self.p3_return_weight)) <= 1e-12:
            payload.pop("p3_return_weight", None)
        return payload
...
class PlanningRequest:
    ...
    p3_return_rows: MatrixRows | None = None
    ...
    def validate(self) -> None:
        ...
        if str(self.information_mode) not in {"p0_p1_p2", "p0_p1_p2_p3", "p0_only", "p0_p1"}:
        ...
        if str(self.information_mode) == "p0_p1_p2_p3":
            if str(self.planning_track) != "runtime_lookahead":
                raise ValueError("P0123 is advisory lookahead, not four-phase execution")
            if str(self.p2_semantics) != "advisory_hint":
                raise ValueError("P0123 requires advisory P2 semantics")
            if self.p3_return_rows is None:
                raise ValueError("P0123 requires p3_return_rows")
            _validate_matrix("p3_return_rows", self.p3_return_rows, world_size=world_size)
            expected_p3 = tuple(
                tuple(int(self.prediction_hint.target_dispatch_rows[col][row]) for col in range(world_size))
                for row in range(world_size)
            )
            if tuple(tuple(int(v) for v in row) for row in self.p3_return_rows) != expected_p3:
                raise ValueError("P0123 requires p3_return_rows == transpose(P2 hint)")
        elif self.p3_return_rows is not None:
            raise ValueError("p3_return_rows is only legal for P0123")
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "p3_return_rows": None if self.p3_return_rows is None else [list(row) for row in self.p3_return_rows],
        }
    ...
    def semantic_payload(self) -> dict[str, Any]:
        ...
        # Keep the exact pre-extension P012 payload when P3 is absent.
        if self.p3_return_rows is not None:
            payload["p3_return_rows"] = [list(row) for row in self.p3_return_rows]
        return payload
