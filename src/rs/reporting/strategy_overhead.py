...
def run_overhead_audit(run_a_dir: Path, run_c_dir: Path | None) -> dict[str, Any]:
    ...
    birkhoff = strategies.get("birkhoff_bucket_phase_local")
    for row in strategies.values():
        if birkhoff is None or row["strategy"] == "birkhoff_bucket_phase_local":
            row["benefit_vs_overhead"] = {
                "transport_delta_vs_birkhoff_us": 0.0 if row["strategy"] == "birkhoff_bucket_phase_local" else None,
                "transport_delta_reason": None,
                "scheduling_overhead_delta_vs_birkhoff_us": 0.0 if row["strategy"] == "birkhoff_bucket_phase_local" else None,
                "total_delta_vs_birkhoff_us": 0.0 if row["strategy"] == "birkhoff_bucket_phase_local" else None,
    ...
    joint = strategies.get("routersense_current_p012_joint_event_rscf_async")
...
__all__ = ["render_markdown", "run_overhead_audit"]
