...
def _load_component_mapping(reference: object) -> dict[str, Any]:
    if not reference:
        return {}
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"component config must be a mapping: {candidate}")
    return dict(payload)


def model_config(model: dict[str, Any], output_dir: Path) -> str:
    path = output_dir / "generated_configs" / "model.yaml"
    reference = model.get("component") or model.get("config") or model.get("path")
    component = _load_component_mapping(reference)
    values = dict(component)
    values.update({key: value for key, value in model.items() if key not in {"component", "config", "path"}})
    raw_path = str(model.get("path", ""))
    if raw_path and not _load_component_mapping(raw_path):
        values.setdefault("local_path", raw_path)
    payload = {
        "model_id": str(values.get("model_id", values.get("local_path", ""))),
        "local_path": str(values.get("local_path", "")),
        "precision": str(values.get("precision", "bf16")),
        "device_index": int(values.get("device_index", 0)),
        "max_new_tokens": int(values.get("max_new_tokens", 32)),
        "default_prompt": str(values.get("default_prompt", "")),
        "trace_layer_path": str(values.get("trace_layer_path", "auto")),
        "trust_remote_code": bool(values.get("trust_remote_code", False)),
    }
    if not payload["model_id"]:
        raise ValueError("model config must resolve model_id or local_path")
    dump_yaml(path, payload)
    return str(path)


def topology_config(topology: dict[str, Any], output_dir: Path) -> str:
    component = _load_component_mapping(topology.get("component") or topology.get("config"))
...
def single_strategy_config(
    *,
    ...
    prediction = dict(comparison.get("prediction", {}) or {})
    ...
    if public_surface:
        ...
        planner_id = str(strategy_runtime.get("planner_id", ""))
        ...
        online_p2_predictor = str(prediction.get("name", strategy_runtime.get("online_p2_predictor", "copy_current_dispatch")))
        ...
        planning_horizon = str(strategy_runtime.get("planning_horizon", "p012"))
        planning_timing = str(strategy_runtime.get("planning_timing", "legacy_auto"))
        p3_return_weight = float(strategy_runtime.get("p3_return_weight", 0.0))
        ...
        planner_id = str(strategy.get("planner_id", ""))
        is_native_baseline = not policy_name
        ...
        planning_horizon = str(strategy.get("planning_horizon", execution.get("planning_horizon", "p012")))
        planning_timing = str(strategy.get("planning_timing", execution.get("planning_timing", "legacy_auto")))
        p3_return_weight = float(strategy.get("p3_return_weight", execution.get("p3_return_weight", 0.0)))
    ...
    config = {
        ...
        "online_policy": {
            ...
            "parameters": {
                ...
                "p3_return_weight": float(p3_return_weight),
                "planning_horizon": str(planning_horizon),
                "planning_timing": str(planning_timing),
                "online_p2_predictor": online_p2_predictor,
                "online_p2_predictor_config": dict(
                    prediction.get("config", execution.get("online_p2_predictor_config", {})) or {}
                ),
            },
