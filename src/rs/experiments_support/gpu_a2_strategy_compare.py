...
def _metric_series(rank_summaries_or_rank0: dict | list[dict], run_dir: Path) -> dict[str, list[float]]:
    ...
    for idx, row in enumerate(repeat_records):
        ...
        metrics["joint_build_us"].append(float(row.get("joint_build_us", 0.0) or 0.0))
        metrics["local_build_us"].append(float(row.get("local_build_us", 0.0) or 0.0))
...
def _expected_count_contract(
    *,
    ...
    if str(strategy) == "routersense_current_p012_joint_event_rscf_async":
        raw_per_rank = _actual_list("joint_build_count")
        ...
        for item in rank_summaries:
            ...
            by_layer = stable_layer_count_map(dict(item.get("joint_build_count_by_layer_per_rank") or item.get("joint_build_count_by_layer") or {}))
            for layer_id in selected_layers:
                ...
                if not valid:
                    ...
                    reasons.append(f"joint_build_count_by_layer:rank={rank}:layer={layer_id}:upper={forward_count}:actual={actual}")
        for index, actual in enumerate(raw_per_rank):
            rank = int(rank_summaries[index].get("rank", index))
            if actual is None:
                reasons.append(f"joint_build_count:rank={rank}:missing")
            elif int(actual) > expected_selected_per_rank:
                reasons.append(f"joint_build_count:rank={rank}:upper={expected_selected_per_rank}:actual={actual}")
        if raw_all is not None and raw_all > expected_selected_all:
            reasons.append(f"joint_build_count_all_rank:upper={expected_selected_all}:actual={raw_all}")
        payload.update(
            {
                "expected_joint_build_upper_bound_per_rank": expected_selected_per_rank,
                "actual_joint_build_count_per_rank": raw_per_rank,
                "joint_build_count_per_rank_valid": all(value is not None and int(value) <= expected_selected_per_rank for value in raw_per_rank),
                "expected_joint_build_upper_bound_all_rank": expected_selected_all,
                "actual_joint_build_count_all_rank": raw_all,
                "joint_build_count_all_rank_valid": raw_all is not None and raw_all <= expected_selected_all,
                "joint_build_count_by_layer_per_rank_valid": bool(raw_layer_valid),
                "joint_build_count_by_layer_per_rank_contract": raw_layer_payload,
            }
        )
...
def aggregate_hotpath_rank_counts(
    ...
    *,
    ...
    if str(strategy) in {"routersense_current_p012_local_event_rscf_async", "routersense_current_p012_joint_event_rscf_async"}:
...
def _build_strategy_result(*, strategy: str, run_dir: Path, summary_payload: dict, c2_passed: bool) -> dict:
    ...
    metrics["local_build_count"] = int(rank0_summary.get("local_build_count", 0) or 0)
    ...
    is_current_joint = str(strategy) == "routersense_current_p012_joint_event_rscf_async"
    checks = {
        ...
        "safe_projection_mode_matches": (
            ...
            or (
                ...
                and int(metrics["local_build_count"]) > 0
    ...
    if is_safe:
        ...
        checks["safe_build_counts"] = int(metrics["local_build_count"]) > 0 and int(metrics["host_projection_count"]) > 0
    else:
    ...
    payload.update(
        {
            ...
            "joint_scope_gain": _safe_gain(by_name, "routersense_current_p012_joint_event_rscf_async", "birkhoff_phase_local_async_p2p"),
            "future_timing_gain": _safe_gain(by_name, "routersense_future_p012_joint_event_rscf_async", "routersense_current_p012_joint_event_rscf_async"),
            "safe_prediction_gain": None,
            "safe_zero_gain": None,
            "global_engine_gain": _safe_gain(by_name, "routersense_future_p012_joint_global_rscf_async", "routersense_future_p012_joint_event_rscf_async"),
            "future_global_gain_vs_native": _safe_gain(by_name, "routersense_future_p012_joint_global_rscf_async", "native"),
        }
    )
    write_json(output_dir / "a2_runner_summary.json", payload)
    write_runner_result_bundle(output_dir, runner_name="run_gpu_a2_strategy_compare", payload=payload, run_kind="GPU_PERFORMANCE")
