...
from rs.prediction import resolve_predictor_id
from rs.runtime.online.megatron_ep.target_planning.fate_megatron import (
    build_megatron_fate_context_provider,
)
from rs.runtime.online.megatron_ep.observation import RouterSenseObserver, parse_layer_id
...
def attach_dispatch_facade(
    *,
    ...
    canonical_predictor_id = resolve_predictor_id(
        str(getattr(config, "online_p2_predictor", "copy_current_dispatch") or "copy_current_dispatch")
    )
    if canonical_predictor_id == "fate_cross_layer_gate" and expert_route_context_provider is None:
        try:
            group_rank = tuple(int(value) for value in ep_group_ranks).index(int(rank))
            expert_route_context_provider = build_megatron_fate_context_provider(
                model=model,
            ...
            raise FormalRuntimeAttachPreflightError(
                f"faithful FATE runtime adapter unavailable: {type(exc).__name__}: {exc}"
            ) from exc
    runtime = RouterSenseInjectionRuntime(
        ...
        expert_route_context_provider=expert_route_context_provider,
    )
    ...
    phase_policy_name = (
        str(resolved_online_policy.builder_key)
        if resolved_online_policy is not None
        else "prepared_priority"
        if str(getattr(config, "planner_id", "") or "")
        else ""
    )
    if phase_policy_name and config.execution_mode in {"phase_sync_wave", "joint_window_async_p2p"} and sample_dispatcher is not None:
        try:
    ...
    try:
        ...
        for name, module in model.named_modules():
            ...
            def wrapped_dispatch(*args: Any, _facade=dispatch_facade, _dispatcher=dispatcher, _name=name, _role=layer_role, **kwargs: Any):
                ...
                try:
                    if runtime.expert_route_context_provider is not None:
                        runtime.expert_route_context_provider.capture(
                            source_layer_id=str(parse_layer_id(str(_name))),
...
def attach_formal_online_runtime(
    *,
    ...
    injection_config = RouterSenseInjectionConfig(
        ...
        planner_id=str(getattr(runtime_config.policy_parameters, "planner_id", "") or ""),
        ...
        p3_return_weight=float(getattr(runtime_config.policy_parameters, "p3_return_weight", 0.0)),
        planning_horizon=str(getattr(runtime_config.policy_parameters, "planning_horizon", "p012")),
        planning_timing=str(getattr(runtime_config.policy_parameters, "planning_timing", "legacy_auto")),
        ...
        diagnostic_compiler_fallback=bool(runtime_config.observation.get("diagnostic_compiler_fallback", False)),
