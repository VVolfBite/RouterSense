...
from .warmup import ensure_compiled_kernel_warmup
...
class TargetLayerPlanningRequest:
    ...
    def validate(self) -> None:
        ...
        if str(self.planning_timing) != "previous_layer":
            raise ValueError("target-layer planning requests must use previous_layer timing")
        if str(self.information_mode) != "p0_p1_p2":
            raise ValueError("future-p012 must reuse the frozen P012 planning horizon")
...
class TargetLayerPlannerMetrics:
    ...
    joint_candidate_us: float = 0.0
    local_fallback_us: float = 0.0
...
class TargetLayerPlannerService:
    ...
    two_horizon_predictor_request_factory: Callable[[TargetLayerPlanningRequest], Any] | None = None
    two_horizon_predictor_config: object | None = None
    ...
    _generation_floor_by_stream: dict[tuple[str, str], int] = field(init=False, default_factory=dict)
    ...
    def start(self) -> None:
        ...
        with self._lock:
            ...
            self._generation_floor_by_stream.clear()
        try:
            warmup = ensure_compiled_kernel_warmup()
        except BaseException as exc:
            self._closed = True
            self._last_error = exc
            self._timeline.append(
                {
                    "event": "compiled_kernel_warmup",
                    "status": "failed",
        ...
        self._timeline.append(
            {
                "event": "compiled_kernel_warmup",
                "status": str(warmup["status"]),
                "duration_us": float(warmup["duration_us"]),
                "planner_ids": list(warmup["planner_ids"]),
                "service_session_id": int(self._service_session_id),
            }
        )
    ...
    @staticmethod
    def _stream_key_from_request(request: TargetLayerPlanningRequest) -> tuple[str, str]:
        return (str(request.run_id), str(request.microbatch_id))
    ...
    def submit(self, request: TargetLayerPlanningRequest) -> PreparationSubmitResult:
        ...
        with self._lock:
            ...
            floor = int(self._generation_floor_by_stream.get(self._stream_key_from_request(request), 0))
            if int(request.forward_epoch) < floor:
    ...
    def cancel_before_generation(self, *, run_id: str, microbatch_id: str, current_generation: int) -> None:
        with self._lock:
            ...
            self._generation_floor_by_stream[stream_key] = floor
            generation_keys = {
                (str(run_id), int(epoch), str(microbatch_id))
                for (_run_id, epoch, _microbatch_id) in self._cancelled_generations
                if str(_run_id) == str(run_id)
                and str(_microbatch_id) == str(microbatch_id)
                and int(epoch) < floor
            }
            for task_key, item in list(self._pending_by_key.items()):
                if (
                    str(item.request.run_id) == str(run_id)
                    and str(item.request.microbatch_id) == str(microbatch_id)
                    and int(item.request.forward_epoch) < floor
                ):
                    generation_keys.add(self._generation_key(item.request))
                    self._pending_by_key.pop(task_key, None)
                    self._queued_keys.discard(task_key)
            for task_key, item in list(self._inflight_by_key.items()):
                if (
                    str(item.request.run_id) == str(run_id)
                    and str(item.request.microbatch_id) == str(microbatch_id)
                    and int(item.request.forward_epoch) < floor
            ...
            for slot_digest, candidate in list(self._publication_state_by_slot.items()):
                if (
                    str(candidate.slot.run_id) == str(run_id)
                    and str(candidate.slot.microbatch_id) == str(microbatch_id)
                    and int(candidate.slot.forward_generation) < floor
                ):
                    self._publication_state_by_slot[slot_digest] = LocalPublicationCandidate(
                        slot=candidate.slot,
                        planner_id=str(candidate.planner_id),
            ...
            self._cancelled_generations.update(generation_keys)
            ...
            try:
                ...
                with self._lock:
                    ...
                    if (
                        ...
                        or int(request.forward_epoch) < floor
                ...
                with self._lock:
                    ...
                    expired = int(request.forward_epoch) < int(self._generation_floor_by_stream.get(self._stream_key_from_request(request), 0))
                    if not newer_pending_exists and not stale_session and not cancelled and not expired:
                        publish_failed = True
                        self._publication_state_by_slot[str(slot.semantic_digest())] = LocalPublicationCandidate(
                            slot=slot,
                            planner_id=str(request.policy_id),
    ...
    def _select_candidate_plans(
        ...
        *,
        ...
        selector = PlannerSelector(
            ...
            joint_planner=(
                self._make_planner("current:p012:joint:event:rscf")
                if joint_plan is None
                else self._make_planner(getattr(joint_plan, "planner_id", "current:p012:joint:event:rscf"))
            ),
    ...
    def _build_target_plan(
        ...
        *,
        ...
        if self.two_horizon_predictor_request_factory is not None:
            predictor = self.two_horizon_predictor_request_factory(request)
        else:
            predictor_factory = self.two_horizon_predictor_factory or (
                lambda predictor_name: SharedTwoHorizonPredictor(
                    predictor_name=predictor_name,
                    predictor_config=self.two_horizon_predictor_config,
                )
            )
        ...
        joint_candidate_start = time.perf_counter_ns()
        joint_policy_id = str(request.joint_planner_id or request.policy_id)
        joint_candidate_planner = self._make_planner(joint_policy_id)
        joint_candidate_plan = joint_candidate_planner.plan(planning_request)
        joint_candidate_logical_plan = _compat_logical_plan_from_window_plan(joint_candidate_plan)
        joint_candidate_end = time.perf_counter_ns()
        metrics.joint_candidate_us = (joint_candidate_end - joint_candidate_start) / 1000.0
        local_fallback_logical_plan = joint_candidate_logical_plan
        estimator = CommonCorePlanEstimator()
        cost_model = PlanningCostModel(
            expert_compute_delay=float(planning_request.constraints.expert_compute_delay),
            full_duplex=bool(planning_request.topology.full_duplex),
        ...
        joint_candidate_score = estimator.estimate(joint_candidate_plan, planning_request, cost_model)
        if not bool(joint_candidate_score.valid):
            raise RuntimeError(f"joint_candidate_plan_invalid:{joint_candidate_score.reason or 'unknown'}")
        validate_window_plan_for_request(joint_candidate_plan, planning_request)
        local_fallback_makespan = float(joint_candidate_score.estimated_makespan)
        selected_variant = "joint_candidate"
        selected_plan = joint_candidate_plan
        selected_score = joint_candidate_score
        safe_selection_us = 0.0
        local_fallback_us = 0.0
        if safe_projection_mode == "host_select":
            local_fallback_start = time.perf_counter_ns()
            paired_policy_id = str(request.local_planner_id or "")
            if not paired_policy_id:
                raise RuntimeError("safe target planner missing local_planner_id")
            local_planner = self._make_planner(paired_policy_id)
            local_plan = local_planner.plan(planning_request)
            validate_window_plan_for_request(local_plan, planning_request)
            local_fallback_logical_plan = _compat_logical_plan_from_window_plan(local_plan)
            local_fallback_end = time.perf_counter_ns()
            local_fallback_us = (local_fallback_end - local_fallback_start) / 1000.0
            metrics.local_fallback_us = float(local_fallback_us)
            safe_start = time.perf_counter_ns()
            selected = self._select_candidate_plans(
                planning_request=planning_request,
                local_plan=local_plan,
                joint_plan=joint_candidate_plan,
                mode=PlannerSelectionMode.COMPARE,
            )
            selected_variant = "local_fallback" if selected.selected_plan.planner_id == local_plan.planner_id else "joint_candidate"
            ...
            local_fallback_makespan = float(selected.local_score.estimated_makespan if selected.local_score is not None else 0.0)
        ...
        local_plan_was_built = safe_projection_mode == "host_select"
        local_plan_was_scored = local_plan_was_built
        local_plan_was_selected = selected_variant == "local_fallback"
        joint_plan_was_selected = selected_variant == "joint_candidate"
        plan = TargetLayerPreparedJointPlan(
            source_layer_id=str(request.source_layer_id),
            target_layer_id=str(request.target_layer_id),
            run_id=request.run_id,
            ...
            logical_payload_digest=str(legacy_logical_digest),
            policy=str(joint_policy_id if selected_variant == "joint_candidate" else (request.local_planner_id or joint_policy_id)),
            weights={
            ...
            joint_candidate_logical_plan_digest=str(joint_candidate_plan.semantic_digest()),
            local_fallback_logical_plan_digest=(
                ""
                if safe_projection_mode != "host_select"
                else str(local_plan.semantic_digest())
            ),
            selected_logical_plan_digest=str(logical_digest),
            joint_candidate_estimated_makespan=float(joint_candidate_score.estimated_makespan),
            local_fallback_estimated_makespan=0.0 if not local_plan_was_scored else float(local_fallback_makespan),
            joint_build_us=float(metrics.joint_candidate_us),
            local_build_us=float(local_fallback_us),
            safe_selection_us=float(safe_selection_us),
            joint_plan_was_built=True,
            joint_plan_was_scored=True,
            joint_plan_was_selected=joint_plan_was_selected,
            local_plan_was_built=local_plan_was_built,
            local_plan_was_scored=local_plan_was_scored,
            local_plan_was_selected=local_plan_was_selected,
            planning_horizon="p012",
            planning_timing=str(request.planning_timing),
            execution_scope="target_p0_p1",
        )
