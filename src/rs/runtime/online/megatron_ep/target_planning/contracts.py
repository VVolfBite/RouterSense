...
class TargetLayerPreparedJointPlan:
    ...
    selected_variant: str = "joint_candidate"
    joint_candidate_logical_plan_digest: str = ""
    local_fallback_logical_plan_digest: str = ""
    selected_logical_plan_digest: str = ""
    joint_candidate_estimated_makespan: float = 0.0
    local_fallback_estimated_makespan: float = 0.0
    joint_build_us: float = 0.0
    local_build_us: float = 0.0
    safe_selection_us: float = 0.0
    joint_plan_was_built: bool = True
    joint_plan_was_scored: bool = True
    joint_plan_was_selected: bool = True
    local_plan_was_built: bool = False
    local_plan_was_scored: bool = False
    local_plan_was_selected: bool = False
    ...
    execution_scope: str = "target_p0_p1"
    ...
    def validate(self) -> None:
        ...
        if str(self.planning_horizon) != "p012":
            raise ValueError("target prepared plans must preserve the P012 horizon")
        if str(self.planning_timing) != "previous_layer":
            raise ValueError("target prepared plans must be generated in the previous layer")
        if str(self.execution_scope) != "target_p0_p1":
            raise ValueError("future-p012 executes only target P0/P1")
        ...
        if self.window_plan is not None:
            ...
            if str(self.logical_payload_digest) and str(self.logical_payload_digest) != canonical_legacy_digest:
                raise ValueError("logical_payload_digest does not match logical_plan payload")
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "TargetLayerPreparedJointPlan":
        ...
        return cls(
            ...
            logical_payload_digest=str(payload.get("logical_payload_digest", "")),
            ...
            selected_variant=str(payload.get("selected_variant", "joint_candidate")),
            joint_candidate_logical_plan_digest=str(payload.get("joint_candidate_logical_plan_digest", "")),
            local_fallback_logical_plan_digest=str(payload.get("local_fallback_logical_plan_digest", "")),
            selected_logical_plan_digest=str(payload.get("selected_logical_plan_digest", "")),
            joint_candidate_estimated_makespan=float(payload.get("joint_candidate_estimated_makespan", 0.0)),
            local_fallback_estimated_makespan=float(payload.get("local_fallback_estimated_makespan", 0.0)),
            joint_build_us=float(payload.get("joint_build_us", 0.0)),
            local_build_us=float(payload.get("local_build_us", 0.0)),
            safe_selection_us=float(payload.get("safe_selection_us", 0.0)),
            joint_plan_was_built=bool(payload.get("joint_plan_was_built", True)),
            joint_plan_was_scored=bool(payload.get("joint_plan_was_scored", True)),
            joint_plan_was_selected=bool(payload.get("joint_plan_was_selected", True)),
            local_plan_was_built=bool(payload.get("local_plan_was_built", False)),
            local_plan_was_scored=bool(payload.get("local_plan_was_scored", False)),
            local_plan_was_selected=bool(payload.get("local_plan_was_selected", False)),
            ...
            planning_horizon=str(payload.get("planning_horizon", "p012")),
            planning_timing=str(payload.get("planning_timing", "previous_layer")),
            execution_scope=str(payload.get("execution_scope", "target_p0_p1")),
        )
