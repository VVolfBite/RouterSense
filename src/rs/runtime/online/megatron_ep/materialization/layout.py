...
from rs.core.contracts.execution import MaterializationValidationResult, MaterializedPlan
from rs.runtime.online.megatron_ep.phase import PhaseReadyContext


def validate_materialized_layout(plan: MaterializedPlan, context: PhaseReadyContext) -> MaterializationValidationResult:
    try:
        plan.validate()
    ...
    for batch in plan.batches:
        ...
        for item in batch.slices:
            if item.task_id in seen_task_ids:
                return MaterializationValidationResult(valid=False, stage="task_id", reason="duplicate_task_id")
            seen_task_ids.add(str(item.task_id))
            if str(item.payload_role) not in role_specs:
                return MaterializationValidationResult(valid=False, stage="payload_role", reason="payload_role_mismatch")
            if int(item.src_group_rank) < 0 or int(item.src_group_rank) >= int(world_size):
                return MaterializationValidationResult(valid=False, stage="rank", reason="src_group_rank_out_of_range")
            if int(item.dst_group_rank) < 0 or int(item.dst_group_rank) >= int(world_size):
                return MaterializationValidationResult(valid=False, stage="rank", reason="dst_group_rank_out_of_range")
            try:
                if int(item.src_global_rank) != int(plan.rank_map.group_rank_to_global_rank(int(item.src_group_rank))):
                    return MaterializationValidationResult(valid=False, stage="rank", reason="src_global_rank_mismatch")
                if int(item.dst_global_rank) != int(plan.rank_map.group_rank_to_global_rank(int(item.dst_group_rank))):
                    return MaterializationValidationResult(valid=False, stage="rank", reason="dst_global_rank_mismatch")
            except ValueError:
                return MaterializationValidationResult(valid=False, stage="rank", reason="rank_out_of_range")
            row_key = (
                str(item.task_id),
            ...
            if row_key in seen_rows:
                return MaterializationValidationResult(valid=False, stage="coverage", reason="duplicate_slice")
            ...
            if int(item.src_global_rank) == int(plan.local_global_rank):
                ...
                if int(item.peer_send_offset_rows) < 0 or end > len(coverage):
                    return MaterializationValidationResult(valid=False, stage="send_offsets", reason="send_offset_out_of_bounds")
                for index in range(int(item.peer_send_offset_rows), end):
                    coverage[index] += 1
                segment = outgoing_segments.get(int(item.dst_global_rank))
                if segment is None:
                    return MaterializationValidationResult(valid=False, stage="physical_send_offsets", reason="missing_outgoing_segment")
                ...
                if int(item.physical_send_offset_rows) != expected_physical:
                    return MaterializationValidationResult(valid=False, stage="physical_send_offsets", reason="physical_send_offset_mismatch")
                ...
                if physical_end > int(segment.send_offset_rows) + int(segment.row_count):
                    return MaterializationValidationResult(valid=False, stage="physical_send_offsets", reason="physical_send_offset_out_of_bounds")
            ...
            if int(item.dst_global_rank) == int(plan.local_global_rank):
                ...
                if int(item.peer_recv_offset_rows) < 0 or end > len(coverage):
                    return MaterializationValidationResult(valid=False, stage="recv_offsets", reason="recv_offset_out_of_bounds")
                for index in range(int(item.peer_recv_offset_rows), end):
                    coverage[index] += 1
                slot = incoming_slots.get(int(item.src_global_rank))
                if slot is None:
                    return MaterializationValidationResult(valid=False, stage="physical_recv_offsets", reason="missing_incoming_slot")
                ...
                if int(item.physical_recv_offset_rows) != expected_physical:
                    return MaterializationValidationResult(valid=False, stage="physical_recv_offsets", reason="physical_recv_offset_mismatch")
                ...
                if physical_end > int(slot.receive_offset_rows) + int(slot.row_count):
                    return MaterializationValidationResult(valid=False, stage="physical_recv_offsets", reason="physical_recv_offset_out_of_bounds")
        ...
        if len({peer for peer, _flow_id in local_outgoing_flow_peers}) > 1:
            return MaterializationValidationResult(valid=False, stage="batch_port", reason="multiple_outgoing_same_wave")
        if len({peer for peer, _flow_id in local_incoming_flow_peers}) > 1:
            return MaterializationValidationResult(valid=False, stage="batch_port", reason="multiple_incoming_same_wave")
    for role, peer_map in send_coverage.items():
        for peer, coverage in peer_map.items():
            if any(value != 1 for value in coverage):
                ...
                return MaterializationValidationResult(valid=False, stage="send_offsets", reason=reason, details={"role": role, "peer_group_rank": int(peer)})
    for role, peer_map in recv_coverage.items():
        for peer, coverage in peer_map.items():
            if any(value != 1 for value in coverage):
                ...
                return MaterializationValidationResult(valid=False, stage="recv_offsets", reason=reason, details={"role": role, "peer_group_rank": int(peer)})
    for role, peer_map in physical_send_coverage.items():
        for peer, coverage in peer_map.items():
            if any(value != 1 for value in coverage):
                ...
                return MaterializationValidationResult(valid=False, stage="physical_send_offsets", reason=reason, details={"role": role, "peer_global_rank": int(peer)})
    for role, peer_map in physical_recv_coverage.items():
        for peer, coverage in peer_map.items():
            if any(value != 1 for value in coverage):
                ...
                return MaterializationValidationResult(valid=False, stage="physical_recv_offsets", reason=reason, details={"role": role, "peer_global_rank": int(peer)})
