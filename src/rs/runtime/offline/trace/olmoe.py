...
def _summarize_weights(weights_seen: list[float]) -> dict[str, float]:
    if not weights_seen:
        return {"mean": 0.0, "min": 0.0, "max": 0.0}
    return {
        "mean": float(sum(weights_seen) / len(weights_seen)),
        "min": float(min(weights_seen)),
        "max": float(max(weights_seen)),
    }
...
def collect_full_sequence_trace(
    ...
    *,
    ...
    hidden_state_payload = {int(layer_id): hidden_states[int(layer_id)].detach().float().cpu() for layer_id in moe_layer_ids}
    ...
    for logical_layer_index, logits in enumerate(router_logits_by_layer):
        ...
        for token_position in range(layer_logits.shape[0]):
            for topk_rank, (weight, expert_id) in enumerate(zip(weights[token_position].tolist(), experts[token_position].tolist(), strict=False)):
                records.append(
                    FullSequenceTraceRecord(
                        request_id=request_id,
                        sample_id=sample_id,
    ...
    summary = {
        ...
        "moe_layer_ids": moe_layer_ids,
        "topk": topk,
        "token_count": int(router_logits_by_layer[0].shape[0]) if router_logits_by_layer else 0,
