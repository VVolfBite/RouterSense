...
def _generate_exact_instances(target_count: int) -> list[ExactInstance]:
    return generate_exact_instances(int(target_count))
...
def _solve_exact_oracle(
    ...
    *,
    ...
    return solve_exact_instance(
        instance,
        scope=str(mode),
        expert_compute_delay=float(compute_delay),
        time_limit_ms=max(1, int(float(time_limit_s) * 1000.0)),
    )
...
def _evaluate_exact_instance_policy(instance: ExactInstance, policy_name: str, hint_type: str, planning_hint: Matrix) -> float:
    ...
    policy = resolve_policy(policy_name=policy_name, bucket_rows=1)
    plan = policy.build_logical_plan(problem)
    audit = replay_and_audit_logical_plan(problem, plan)
