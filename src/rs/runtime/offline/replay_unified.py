...
from rs.scheduling.p012_future._kernel.event_core import bind_template
from rs.scheduling.p012_future._kernel.plan import tuple_to_plan as kernel_tuple_to_plan
...
def _bind_formal_plan_to_execution_truth(
    *,
    formal_plan,
    ...
    max_waves: int,
) -> tuple[LogicalSchedulePlan, str | None]:
    """Bind advisory P2 template slots to actual P2 rows before replay.

    The formal planner never marks predicted P2 bytes executable. Offline
    evaluation therefore consumes the exact same forecast order as runtime,
    then applies the shared event-kernel truth binder. This keeps planning
    information separated from execution truth and prevents incomplete P2
    plans from being scored against exact oracles.
    """
    has_executable_p2 = any(
        str(flow.phase) == "p2_next_dispatch" and bool(flow.executable)
        for wave in formal_plan.waves
        for flow in wave.flows
    )
    actual_p2_present = any(
        int(value) > 0 for row in replay_window.p2_truth_rows for value in row
    )
    has_planned_p2 = any(
        str(flow.phase).startswith("p2_")
        for wave in formal_plan.waves
        for flow in wave.flows
    )
    if has_executable_p2:
        return _compat_logical_plan_from_window_plan(formal_plan), None
    if not actual_p2_present and not has_planned_p2:
        return _compat_logical_plan_from_window_plan(formal_plan), None

    world = int(replay_window.group_size)
    wave_count = len(formal_plan.waves)
    phase = np.full((wave_count, world), -1, dtype=np.int8)
    destination = np.full((wave_count, world), -1, dtype=np.int16)
    size = np.zeros((wave_count, world), dtype=np.int32)
    duration = np.zeros(wave_count, dtype=np.float64)
    starts = np.zeros(wave_count, dtype=np.float64)
    cursor = 0.0
    phase_map = {
        "p0_dispatch": 0,
        "p1_return": 1,
        "p2_next_dispatch": 2,
        "p2_next_dispatch_forecast": 2,
    }
    for wave_index, wave in enumerate(formal_plan.waves):
        starts[wave_index] = cursor
        duration[wave_index] = float(wave.estimated_duration)
        cursor += float(wave.estimated_duration)
        for flow in wave.flows:
            source = int(flow.src_rank)
            if phase[wave_index, source] >= 0:
                raise ValueError("formal matching wave has multiple outgoing flows for one source")
            phase[wave_index, source] = int(phase_map[str(flow.phase)])
            destination[wave_index, source] = int(flow.dst_rank)
            size[wave_index, source] = int(flow.row_count)
    ...
    template = (
        ...
        destination,
        size,
        np.zeros(wave_count, dtype=np.int32),
        duration,
        starts,
        starts + duration,
    ...
    actual = [
        np.asarray(replay_window.p0_truth_rows, dtype=np.int32),
        np.asarray(replay_window.p1_truth_rows, dtype=np.int32),
        np.asarray(replay_window.p2_truth_rows, dtype=np.int32),
    ]
    hint = np.asarray(planning_hint.p2_hint_rows, dtype=np.int32)
    bound = bind_template(
        actual,
        hint,
        template,
    ...
    kernel_plan = kernel_tuple_to_plan(
        bound[:13],
        planner_id=str(formal_plan.planner_id),
        planner_family=str(formal_plan.planner_family),
        branch="offline_truth_bind",
        request_digest=str(formal_plan.request_digest),
        forecast=False,
        metadata={**dict(formal_plan.metadata), "offline_truth_bound": True},
    )
    raw_schedule: list[dict[str, Any]] = []
    logical_waves: list[LogicalWave] = []
    phase_id = {
        "p0_dispatch": 0,
        "p1_return": 1,
        "p2_next_dispatch": 2,
    }
    for wave in kernel_plan.waves:
        logical_flows: list[FlowDemand] = []
        for flow in wave.flows:
            logical_flow = FlowDemand(
            ...
            raw_schedule.append(
                {
                    "chunk_id": str(flow.segment_id),
                    "flow_id": str(flow.parent_flow_id),
                    "phase": int(phase_id[str(flow.phase)]),
                    "src_gpu": int(flow.src_rank),
                    "dst_gpu": int(flow.dst_rank),
                    "start": float(wave.start_time),
                    "end": float(wave.end_time),
                    "served_volume": float(flow.row_count),
                    "wave_id": int(wave.wave_id),
                }
            )
        logical_waves.append(
            LogicalWave(
                wave_id=int(wave.wave_id),
                flows=tuple(logical_flows),
                duration=float(wave.estimated_duration),
            )
        )

    # Schedule-derived diagnostics from the forecast plan are stale after truth
    # binding.  Replace them with values from the bound execution plan and keep
    # the exact start/end timeline so expert-compute idle gaps remain auditable.
    bound_diagnostics = {
        key: value
        for key, value in dict(formal_plan.metadata).items()
        if key not in {"raw_schedule", "makespan", "wave_count"}
    }
    bound_diagnostics.update(
        {
            "offline_truth_bound": True,
            "makespan": float(kernel_plan.makespan),
            "wave_count": int(len(kernel_plan.waves)),
            "raw_schedule": raw_schedule,
        }
    )
    logical = LogicalSchedulePlan(
        policy_name=str(formal_plan.planner_id),
        waves=tuple(logical_waves),
        diagnostics=bound_diagnostics,
    )
    return logical, str(kernel_plan.semantic_digest())
...
def _planning_mode_contract(*, scheduling_mode: str, has_p2: bool) -> tuple[str, str]:
    """Return the planner-side information contract for replay.

    ``scheduling_mode`` controls which phases the *execution audit* scores.  It
    must not leak execution truth into the planner request.  Replay therefore
    always plans through the runtime-lookahead contract and binds the realized
    P2 matrix only after the forecast plan has been frozen.  In
    ``execution_window`` mode the audit still executes and validates the real
    P2 phase; only the planner's information boundary remains advisory.
    """
    mode = str(scheduling_mode)
    if mode not in {"execution_window", "runtime_lookahead"}:
    ...
    *,
    ...
    return MultiPhaseSchedulingProblem(
        ...
        p2_next_dispatch_forecast_matrix=(
            execution_truth.p2_truth_rows if mode == "execution_window" else hint.p2_hint_rows
        ),
    )
...
class ReplayEngine:
    ...
    def execute(
        ...
        *,
        ...
        planner = PlannerRegistry.create(str(policy_name), {"runtime_usage": False})
        formal_plan = planner.plan(request)
        logical_plan, bound_plan_digest = _bind_formal_plan_to_execution_truth(
        ...
        return {
            ...
            # The logical-plan identity belongs to the forecast information
            # boundary and therefore must not change when only execution truth
            # changes.  The realized bound plan has its own explicit digest.
            "logical_plan_digest": str(formal_plan.semantic_digest()),
            "forecast_plan_digest": str(formal_plan.semantic_digest()),
            "execution_bound_plan_digest": str(bound_plan_digest or formal_plan.semantic_digest()),
