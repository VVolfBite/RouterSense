...
def _run_policy(problem, policy_name: str, instance: ExactInstance) -> dict[str, Any]:
    if ":" in str(policy_name):
        request = build_window_planning_request(
            identity=PlanningIdentity(
                request_id=f"{instance.instance_id}:{policy_name}",
                run_id=instance.instance_id,
                window_id=f"{instance.instance_id}:0",
                source_layer_id="0",
                target_layer_id="1",
            ),
            p0_dispatch_rows=instance.p0,
            p1_return_rows=instance.p1,
            p2_hint_rows=instance.p2,
            predictor_id="perfect_trace_hint",
            confidence=1.0,
            topology=PlanningTopology(world_size=instance.rank_count),
        ...
        formal_plan = PlannerRegistry.create(str(policy_name), {"runtime_usage": False}).plan(request)
        plan = _compat_logical_plan_from_window_plan(formal_plan)
    else:
    ...
    makespan = float(audit.get("replay_makespan", audit.get("makespan", plan.diagnostics.get("makespan", 0.0))))
    return {
