...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `birkhoff_bvn` | `B_birkhoff`, `B_birkhoff_wave`, `B_barrier_aware_birkhoff`, `B_barrier_aware_birkhoff_wave` | `U_ibbr` | `fast_schedule_birkhoff`, `fast_schedule_birkhoff_wave`, `fast_schedule_barrier_aware_birkhoff`, `fast_schedule_barrier_aware_birkhoff_wave` | `fast_schedule_ibbr` | `implemented` | `pending` | `false` | `legacy_coarse_wave`, `legacy_fluid_reference`, `dynamic_bucket_current` | `B_birkhoff` is also the local oracle-like reference. `U_ibbr` is the closest historical joint extension found in POC1, but it is not yet promoted into current paired evidence mainline. |
...
| `barrier_price_adaptive_matching` | `B_barrier_price_adaptive_matching` | `U_barrier_price_adaptive_matching`, `U_barrier_price_adaptive_matching_atomic` | not found | `fast_schedule_u_barrier_price_adaptive_matching`, `fast_schedule_u_barrier_price_adaptive_matching_atomic` | `pending` | `recoverable` | `false` | `dynamic_bucket_current`, `legacy_atomic_token` | Historical U exists clearly. B-side is not promoted yet because the phase-local adaptive-price semantics need a cleaner formal definition. |
| `lagrangian_cross_phase` | `B_lagrangian_phase_local` | `U_lagrangian` | not found | `fast_schedule_lagrangian` | `pending` | `implemented` | `false` | `legacy_atomic_token` | Historical U exists and is recovered in Tier1. A fair phase-local B-side is not yet promoted. |
