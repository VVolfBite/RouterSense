...
def test_package_source_only_mainline_excludes_legacy_and_runtime(tmp_path):
    ...
    subprocess.run([str(root / "RS" / "tools" / "archive" / "package_source_only.sh"), "--scope", "mainline", str(archive)], check=True)
...
def test_package_source_only_full_includes_legacy(tmp_path):
    ...
    subprocess.run([str(root / "RS" / "tools" / "archive" / "package_source_only.sh"), "--scope", "full", str(archive)], check=True)
    listing = subprocess.check_output(["tar", "-tzf", str(archive)], text=True).splitlines()
...
def test_verify_source_archive_matches_head_mainline(tmp_path):
    ...
    archive = tmp_path / "mainline.tar.gz"
    subprocess.run([str(root / "RS" / "tools" / "archive" / "package_source_only.sh"), "--scope", "mainline", str(archive)], check=True)
    subprocess.run([str(root / "RS" / "tools" / "archive" / "verify_source_archive_matches_head.sh"), "--scope", "mainline", str(archive)], check=True)
