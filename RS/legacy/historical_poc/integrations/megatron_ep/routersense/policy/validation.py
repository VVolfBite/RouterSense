        ...
        for peer_idx, rows in enumerate(obs.per_peer_rows):
            ...
            flows.append(
                PeerFlow(
                    ...
                    is_cross_rank=True,
                    is_cross_node=int(obs.topology.node_index) != int(peer_idx if False else -9999),  # overwritten below
                )
            )
...
def validate_shadow_plan(context: PolicyContext, plan: RouterSensePlan) -> None:
    ...
    for wave in plan.waves:
        ...
        for flow in wave.flows:
            ...
            directed_pairs.add((flow.src_rank, flow.dst_rank))
