    ...
    for batch in plan.batches:
        ...
        for item in batch.slices:
            ...
            if int(item.src_global_rank) == int(plan.local_global_rank):
                ...
                end = int(item.send_offset_rows) + int(item.row_count)
                if int(item.send_offset_rows) < 0 or end > len(coverage):
                    return ValidationResult(valid=False, stage="send_offsets", reason="send_offset_out_of_bounds")
                for index in range(int(item.send_offset_rows), end):
                    coverage[index] += 1
            if int(item.dst_global_rank) == int(plan.local_global_rank):
                ...
                end = int(item.recv_offset_rows) + int(item.row_count)
                if int(item.recv_offset_rows) < 0 or end > len(coverage):
                    return ValidationResult(valid=False, stage="recv_offsets", reason="recv_offset_out_of_bounds")
                for index in range(int(item.recv_offset_rows), end):
                    coverage[index] += 1
