...
def _summary_tensor_length(world_size: int) -> int:
    # header(6) + per_peer_bytes(world_size) + edges(world_size * 4) + preferred_priority(world_size^2)
    return 6 + int(world_size) + (int(world_size) * 4) + (int(world_size) * int(world_size))
...
def _encode_planning_summary_tensor(summary: PhasePlanningSummary, *, world_size: int, device: torch.device) -> torch.Tensor:
    ...
    payload[2] = _PHASE_TO_CODE[str(summary.phase)]
    payload[3] = _HINT_MODE_TO_CODE.get(str(summary.p2_hint.hint_mode), 0)
    payload[4] = len(summary.outgoing_edges)
    payload[5] = int(summary.local_rank)
    cursor = 6
    ...
    for dst_rank in range(world_size):
        ...
        cursor += 4
    preferred = {
        (int(item.get("src_rank", -1)), int(item.get("dst_rank", -1))): int(item.get("priority", -1))
        for item in summary.p2_hint.metadata.get("preferred_edges", ())
        if str(item.get("phase", "")) == str(summary.phase)
    }
    for src_rank in range(world_size):
        for dst_rank in range(world_size):
            payload[cursor] = int(preferred.get((src_rank, dst_rank), -1))
            cursor += 1
...
def _decode_planning_summary_tensor(
    ...
    *,
    ...
    hint_mode = _CODE_TO_HINT_MODE.get(int(values[3]), "none")
    edge_count = int(values[4])
    local_rank = int(values[5])
    ...
    for src_rank in range(world_size):
        for dst_rank in range(world_size):
            priority = int(values[cursor])
            cursor += 1
            if priority < 0:
                continue
            preferred_edges.append(
                {
                    "phase": str(phase),
    ...
    hint = FutureDemandHint(
        hint_mode=hint_mode,
        hint_digest="",
        hint_source=hint_mode,
        metadata={"preferred_edges": preferred_edges},
    )
    return PhasePlanningSummary(
        ...
        per_peer_rows=tuple(0 for _ in range(world_size)),
...
def run_phase_plan_agreement(
    *,
    ...
    full_plan = None
    summary_decode_start_ns = time.monotonic_ns()
    if rank == root_rank:
        rebuilt_contexts: list[PhaseReadyContext] = []
        for item in gathered:
            summary = _decode_planning_summary_tensor(
                item,
    ...
    abstract_decode_start_ns = time.monotonic_ns()
    decoded = (
        abstract_plan
        if isinstance(abstract_plan, AbstractPhaseExecutionPlan)
        else _decode_abstract_plan_tensor(buffer_tensor, local_context=local_context)
    )
    abstract_decode_end_ns = time.monotonic_ns()
    ...
    timing_metrics = {
        ...
        "summary_decode_time_us": (summary_decode_end_ns - summary_decode_start_ns) / 1000.0,
        ...
        "abstract_decode_time_us": (abstract_decode_end_ns - abstract_decode_start_ns) / 1000.0,
