"""Experiment-support helpers for U-weight tuning."""
...
def _build_tuned_policy_plan(problem, *, policy_name: str, residual_weight: float, barrier_weight: float, age_weight: float, prediction_weight: float):
...
def _tune_barrier_criticality(problem, residual_weight: float, barrier_weight: float, age_weight: float, prediction_weight: float):
    return _build_tuned_policy_plan(problem, policy_name="U_barrier_criticality_global_matching", residual_weight=residual_weight, barrier_weight=barrier_weight, age_weight=age_weight, prediction_weight=prediction_weight)


def _tune_gated_maxweight(problem, residual_weight: float, barrier_weight: float, age_weight: float, prediction_weight: float):
    return _build_tuned_policy_plan(problem, policy_name="U_gated_maxweight_matching", residual_weight=residual_weight, barrier_weight=barrier_weight, age_weight=age_weight, prediction_weight=prediction_weight)
...
def run_u_weight_tuning(*, fixture_dir: Path, policies: Iterable[str] | None = None, grid_size: int | None = None) -> dict[str, Any]:
    ...
    for raw_policy in selected_policies:
        ...
        for params in grid:
            ...
            for path in train_paths:
                ...
                problem = build_replay_problem(fixture, mode="runtime_lookahead", p2_source="copy_current_dispatch", expert_compute_delay=0.0)
        ...
        summary["policies"][raw_policy] = {
            ...
            "family_quality_label": "mainline_candidate" if SAFE_BY_RAW[raw_policy] in {"RS_safe_barrier_criticality", "RS_safe_gated_greedy"} else "keep_diagnostic_only",
        }
...
__all__ = ["BUILDERS", "SAFE_BY_RAW", "run_u_weight_tuning"]
