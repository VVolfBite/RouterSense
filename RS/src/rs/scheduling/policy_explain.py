...
from rs.scheduling.multiphase.safe_joint import SafeJointPolicy
...
def explain_policy_decision(
    ...
    *,
    ...
    if isinstance(policy, SafeJointPolicy) or policy_name in _SAFE_TO_RAW_AND_B:
        safe_policy = policy if isinstance(policy, SafeJointPolicy) else resolve_policy(policy_name=policy_name, bucket_rows=0)
        assert isinstance(safe_policy, SafeJointPolicy)
        raw_plan = _collect_debug_plan(safe_policy._raw_u_policy, problem)
        b_plan = safe_policy._paired_b_policy.build_logical_plan(problem)
        safe_plan = LogicalSchedulePlan(
            policy_name=safe_policy.policy_name,
            waves=(),
            diagnostics={},
        )
        ...
        safe_plan = LogicalSchedulePlan(policy_name=safe_policy.policy_name, waves=selected.plan.waves, diagnostics=diagnostics)
