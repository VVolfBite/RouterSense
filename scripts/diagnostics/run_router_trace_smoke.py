...
from rs.runtime.offline.trace.olmoe import collect_olmoe_router_trace
from rs.topology import load_inventory, resolve_inventory_path, resolve_preferred_model_path


def summarize_router_trace(trace: dict) -> dict:
    records = list(trace.get("records", []))
    return {
        "record_count": len(records),
        "layer_count": len({str(record.get("layer_path", "")) for record in records}),
