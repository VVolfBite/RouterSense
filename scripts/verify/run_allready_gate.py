...
def deploy_gate(*, output_dir: Path, timeout_seconds: int) -> dict[str, Any]:
    ...
    commands = {
        ...
        "model_preflight_dry_run": ["bash", "scripts/deploy/verify_mounted_model.sh", "deploy/inventory/hosts.example.yaml"],
        "runtime_environment_dry_run": ["bash", "scripts/deploy/verify_runtime_environment.sh", "deploy/inventory/hosts.example.yaml"],
        "link_calibration_dry_run": ["bash", "scripts/deploy/calibrate_cluster_links.sh", "deploy/inventory/hosts.example.yaml", "--run-id", "allready-gate"],
        "collect_dry_run": ["bash", "scripts/deploy/collect_remote_logs.sh", "deploy/inventory/hosts.example.yaml", "--run-id", "allready-gate"],
        "result_summary_dry_run": ["bash", "scripts/deploy/summarize_collected_run.sh", "deploy/inventory/hosts.example.yaml", "--run-id", "allready-gate"],
        "stop_dry_run": ["bash", "scripts/deploy/stop_remote_jobs.sh", "deploy/inventory/hosts.example.yaml", "--run-id", "allready-gate"],
        "pipeline_dry_run": ["bash", "scripts/deploy/run_deployment_pipeline.sh", "deploy/inventory/hosts.example.yaml", "--run-id", "allready-gate"],
        "strategy_dry_run": [
            sys.executable,
            "-m",
    ...
    expected_fragments = {
        ...
        "model_preflight_dry_run": '"status": "DRY_RUN"',
        "runtime_environment_dry_run": '"status": "DRY_RUN"',
        "link_calibration_dry_run": '"status": "DRY_RUN"',
        "collect_dry_run": '"status": "DRY_RUN"',
        "result_summary_dry_run": '"status": "DRY_RUN"',
