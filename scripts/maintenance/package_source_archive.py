...
def _matches_excluded(relative_posix: str, *, scope: str) -> bool:
    ...
    if scope == "mainline":
        patterns.extend(["legacy", "legacy/*", "tests/legacy", "tests/legacy/*"])
    ...
    for pattern in patterns:
        ...
        if relative_posix.startswith(f"{pattern}/"):
            return True
        if not any(token in pattern for token in "*?[]") and pattern in parts:
            return True
    return False
...
def _copy_tree(staging_root: Path, *, scope: str) -> tuple[list[str], list[str]]:
    ...
    if scope == "mainline":
        excluded_patterns.extend(["legacy", "legacy/*", "tests/legacy", "tests/legacy/*"])
...
def _self_check(archive_path: Path, *, scope: str, archive_format: str) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="rs_source_check_") as tmp:
        ...
        env = dict(os.environ)
        # Nested pytest collection may be invoked while this packager itself is
        # under pytest.  Do not leak parent-run plugin state into the archive
        # self-check; third-party tracing plugins can otherwise keep the child
        # process alive after collection completes.
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="rs_source_stage_") as tmp:
        ...
        if bool(args.skip_self_check):
            initial_self_check = {"status": "skipped", "reason": "skip_self_check", "commands": []}
            repack_check = {"status": "skipped", "reason": "skip_self_check"}
        ...
        _make_archive(staging_root, archive_path, archive_format=archive_format)
        if bool(args.skip_self_check):
            final_self_check = {"status": "skipped", "reason": "skip_self_check", "commands": []}
        elif isinstance(cached_entry, dict):
        ...
        if (
            ...
            _write_cache(cache)
        repack_failed = (not bool(args.skip_self_check)) and (not bool(args.skip_repack_check)) and str(repack_check["status"]) != "passed"
        self_check_failed = (not bool(args.skip_self_check)) and str(final_self_check["status"]) != "passed"
        if self_check_failed or repack_failed:
